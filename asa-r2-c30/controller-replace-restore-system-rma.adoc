...
summary: 要完成 ASA C30 存储系统的控制器更换，请先恢复 NetApp 存储加密配置（如有必要），然后在新控制器上安装所需的许可证。接下来，请确认逻辑接口正在向其主端口报告，并执行集群运行状况检查。最后，注册新控制器的序列号，然后将出现故障的部件返回 NetApp。 
---
= 完成 ASA C30 系统的控制器更换
:allow-uri-read: 
:icons: font
...
[role="lead"]
要完成 ASA C30 存储系统的控制器更换，首先恢复 NetApp 存储加密配置（如有必要）并在新控制器上安装所需的许可证。接下来、确认逻辑接口(Logical Interface、Logical Interface、Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称最后、注册新控制器的序列号、然后将故障部件退回给NetApp。
...
在将 _replacement_ 节点恢复服务之前，您应该验证 LIF 是否位于其主端口上，如果启用了 AutoSupport，则注册 _replacement_ 节点的序列号，检查集群的运行状况，关闭任何维护窗口，并重置自动交还。
