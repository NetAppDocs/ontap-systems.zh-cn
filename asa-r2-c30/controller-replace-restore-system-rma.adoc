...
summary: '要完成 ASA C30 存储系统的控制器更换，首先恢复 NetApp 存储加密配置（如有必要）并在新控制器上安装所需的许可证。接下来、确认逻辑接口(Logical Interface、Logical Interface、Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称最后、注册新控制器的序列号、然后将故障部件退回给NetApp。' 
---
= 完整控制器更换—ASA C30
:allow-uri-read: 
:icons: font
