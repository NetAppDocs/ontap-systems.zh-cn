...
summary: ASA C30 存储系统中的启动介质存储了必要的固件和配置数据。更换过程包括卸下控制器模块、卸下受损的启动介质、安装替代启动介质、然后将ONTAP映像传输到替代启动介质。 
---
= 使用自动启动恢复过程更换 ASA C30 系统中的启动介质
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA C30 存储系统中的启动介质存储了必要的固件和配置数据。更换过程包括卸下控制器模块、卸下受损的启动介质、安装替代启动介质、然后将ONTAP映像传输到替代启动介质。
...
.下一步行动
在物理更换受损的启动介质后，您link:bootmedia-recovery-image-boot-bmr.html["从配对节点还原ONTAP映像"] 。
