...
---
= 更换 ASA A70 或 ASA A90 系统中的 NV 电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
关闭或接管受损的控制器。- 接管并停止受损的控制器，以便健康控制器继续从受损的控制器的存储中提供数据。为此，您需要在 AutoSupport 中禁止自动创建案例，禁用自动回馈，并将受损的控制器带到 LOADER 提示符处。LOADER 提示符是安全停止状态，您可以从中更换 FRU。
