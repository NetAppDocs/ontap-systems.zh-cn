...
summary: 当硬件故障需要时，请更换 ASA A70 或 ASA A90 系统的机箱。更换过程包括卸下控制器和电源装置、卸下驱动器、安装更换机箱和重新安装机箱组件。 
---
= 更换 ASA A70 或 ASA A90 系统中的底盘
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当硬件故障需要更换时，请更换ASA A70 和ASA A90 的机箱。拆下控制器和电源单元（PSU），拆下驱动器，安装替换机箱，然后重新安装所有组件。
...
.下一步是什么？
更换损坏的底盘并重新安装组件后，link:chassis-replace-complete-system-restore-rma.html["完成机箱更换"] 。
