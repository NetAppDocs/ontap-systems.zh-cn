...
summary: 将存储资源的控制权归还给替代控制器、以便ASA A70或ASA A90系统可以恢复正常运行。 
---
= 交还 ASA A70 或 ASA A90 系统中的控制器以恢复正常操作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将存储资源的控制权归还给替代控制器、以便ASA A70或ASA A90系统可以恢复正常运行。恢复过程根据系统使用的加密类型而有所不同：无加密或板载密钥管理器 (OKM) 加密。
...
.板载加密(OKM)
--
恢复并重新密封 OKM 的板载加密密钥，并使控制器恢复正常运行。

.开始之前
在开始之前，请查看 https://docs.netapp.com/us-en/ontap-cli/security-key-manager-onboard-show-backup.html["security key-manager onboard show-backup"]或联系 NetApp 支持部门。

.步骤
. 从Loader提示符处启动到ONTAP菜单 `boot_ontap menu`、然后选择选项10。
. 显示集群的密钥管理备份信息。
+
.. 运行以下命令以显示集群的密钥管理备份信息：
+
...
.. 验证密钥的状态： `security key-manager key query -restored false`
+
正确同步时，输出不应显示任何结果。
...
NOTE: 请勿因丢失密钥而推翻对回馈的否决。请联系 NetApp 支持人员。
