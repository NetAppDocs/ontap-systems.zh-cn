...
summary: 更换 ASA A70 或 ASA A90 系统中的实时时钟电池（通常称为纽扣电池），以确保依赖精确时间同步的服务和应用程序保持运行。 
---
= 更换 ASA A70 或 ASA A90 系统中的实时时钟电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更换ASA A70或ASA A90系统中的实时时钟(RTC)电池(通常称为币形电池)、以确保依赖于准确时间同步的服务和应用程序保持正常运行。
