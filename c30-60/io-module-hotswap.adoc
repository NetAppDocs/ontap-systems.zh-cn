...
summary: 如果模块发生故障并且存储系统满足所有 ONTAP 版本要求，则可以热插拔 AFF C30 或 AFF C60 存储系统中的以太网 I/O 模块。 
---
= 在 AFF C30 或 AFF C60 存储系统中热插拔 I/O 模块
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果模块发生故障并且存储系统满足所有 ONTAP 版本要求，则可以热插拔 AFF C30 或 AFF C60 存储系统中的以太网 I/O 模块。
...
+
** _I/O 模块受损的控制器_是您要热插拔 I/O 模块的控制器。
** _正常控制器_是具有受损 I/O 模块的控制器的 HA 合作伙伴。
