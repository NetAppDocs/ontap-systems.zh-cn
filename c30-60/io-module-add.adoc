...
summary: 将 I/O 模块添加到您的 AFF C30 或 AFF C60 存储系统，以增强网络连接并扩展系统处理数据流量的能力。 
---
= 将 I/O 模块添加到 AFF C30 或 AFF C60 系统
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
向您的 AFF C30 或 AFF C60 存储系统添加 I/O 模块，以增强网络连接并扩展系统处理数据流量的能力。您可以使用此过程将 I/O 模块作为计划升级添加到运行中的控制器，或在维护期间添加到 I/O 模块受损的控制器。

如果存在可用插槽或所有插槽均已完全填充、则可以向AFF C30和AFF C60存储系统添加I/O模块。

.关于此任务
无论控制器的 I/O 模块当前是否正常运行或受损，您都可以在 HA 对中的任何一个控制器上执行此过程。由于无法将 I/O 模块添加到正在运行的控制器，因此必须在安装模块之前关闭目标控制器。HA 合作伙伴在此过程中接管存储操作，以确保客户端能够持续访问数据。

* 如果目标控制器处于*运行*状态（计划添加），请按照所写的关闭步骤操作，在该过程中将目标控制器视为具有受损 I/O 模块。
* 如果目标控制器的 I/O 模块受损，请按照所述关闭步骤操作。
...
接管并关闭目标控制器，以便其 HA 合作伙伴在添加 I/O 模块时继续提供数据。链接的步骤将关闭的控制器称为托管受损 I/O 模块的控制器。无论您是接管控制器以进行计划添加，还是关闭已有受损 I/O 模块的控制器，这些步骤均适用。
...
.开始之前
* 请检查 https://hwu.netapp.com["NetApp Hardware Universe"^]以确保新的 I/O 模块与您的存储系统和正在运行的 ONTAP 版本兼容。
