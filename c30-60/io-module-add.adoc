...
---
= 将 I/O 模块添加到 AFF C30 或 AFF C60 存储系统，以增强网络连接
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将 I/O 模块添加到您的 AFF C30 或 AFF C60 存储系统，以增强网络连接并扩展系统处理数据流量的能力。您可以使用此过程将 I/O 模块作为计划升级添加到操作控制器，或在维护期间添加到受损的控制器。

如果存在可用插槽或所有插槽均已完全填充、则可以向AFF C30和AFF C60存储系统添加I/O模块。

.关于此任务
您可以在 HA 对中的任何一个控制器上执行此过程，无论控制器当前是否正常工作或受损。由于无法将 I/O 模块添加到正在运行的控制器，因此必须在安装模块之前关闭目标控制器。HA 合作伙伴在流程过程中接管存储操作，以便客户能够继续使用数据。

* 如果目标控制器处于 *operational* （计划添加）状态，请按照所写的关闭步骤进行操作，将目标控制器视为该内容中的"受损"控制器。
* 如果目标控制器*受损*，请按照所写的关闭步骤操作。
...
接管并关闭目标控制器模块，以便其 HA 合作伙伴在添加 I/O 模块时继续提供数据。链接的步骤将被关闭的控制器称为"受损"控制器；无论您是接管计划添加的操作控制器还是关闭已受损的控制器，都适用相同的步骤。
