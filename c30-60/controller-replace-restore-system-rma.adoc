...
summary: 要完成AFF C30 或AFF C60 存储系统的控制器更换，请首先恢复NetApp存储加密配置（如有必要），然后在新控制器上安装所需的许可证。接下来，确认逻辑接口 LIF 正在向其主端口报告并执行集群运行状况检查。最后，注册新控制器的序列号，然后将故障部件返回给NetApp 
---
= 完成 AFF C30 或 AFF C60 系统的控制器更换
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要完成AFF C30 或AFF C60 存储系统的控制器更换，首先恢复NetApp存储加密配置（如有必要）并在新控制器上安装所需的许可证。接下来，确认逻辑接口 (LIF) 正在向其主端口报告并执行集群健康检查。最后，注册新控制器的序列号，然后将发生故障的部件退回给NetApp。
