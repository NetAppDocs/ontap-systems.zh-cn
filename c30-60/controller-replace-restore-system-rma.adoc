...
[role="lead"]
还原NetApp存储加密配置(如有必要)、为替代控制器安装许可证、验证逻辑接口(Logical Interface、Logical Interface、lIF)是否向其主端口报告、执行集群运行状况检查、注册替代控制器的序列号、然后将故障部件返回给NetApp以完成AFF C30和C60控制器更换过程中的最后一步。
