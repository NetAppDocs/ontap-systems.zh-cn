...
summary: 在 AFF C30 或 AFF C60 存储系统中安装新的启动介质设备后，您可以启动自动启动介质恢复过程，以从伙伴节点恢复配置。在恢复过程中，系统会检查加密是否已启用，并确定正在使用的密钥加密类型。如果启用了密钥加密，系统将引导您完成相应的恢复步骤。 
---
= 使用自动启动恢复过程在 AFF C30 或 AFF C60 存储系统上启动恢复映像
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 AFF C30 或 AFF C60 存储系统中安装新的启动介质设备后，您可以启动自动启动介质恢复过程，以从伙伴节点恢复配置。在恢复过程中，系统会检查加密是否已启用，并确定正在使用的密钥加密类型。如果启用了密钥加密，系统将引导您完成相应的恢复步骤。
...
.. 前往 <<reenable-auto-giveback,重新启用自动返还功能>> 如果它被禁用了。
...
`key manager is configured.`
 a| 
已安装加密功能。前往<<restore-key-manager,恢复密钥管理器>>。
...
. [[restore-key-manager]] 使用适合您配置的相应过程还原密钥管理器：
+
[role="tabbed-block"]
====
.板载密钥管理器（ OKM ）
...
. [[reenable-auto-giveback]]如果已禁用自动交还，请重新启用：
+
