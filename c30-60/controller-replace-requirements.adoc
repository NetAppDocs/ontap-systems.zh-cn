...
summary: 在更换 AFF C30 或 AFF C60 存储系统中的控制器之前，请确认您符合成功更换的必要要求。这包括验证系统中的所有其他组件是否正常运行，验证您是否拥有正确的替换控制器，并将控制器的控制台输出保存到文本日志文件中。 
---
= 在更换 AFF C30 或 AFF C60 存储系统中的控制器之前，请查看相关要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换 AFF C30 或 AFF C60 存储系统中的控制器之前，请确认您符合成功更换的必要要求。这包括验证系统中的所有其他组件是否正常运行，验证您是否拥有正确的替换控制器，并将控制器的控制台输出保存到文本日志文件中。
