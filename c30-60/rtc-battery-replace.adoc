...
summary: 在 AFF C30 或 AFF C60 存储系统中更换实时时钟电池（通常称为纽扣电池），以确保依赖精确时间同步的服务和应用程序保持运行。 
---
= 更换 AFF C30 或 AFF C60 存储系统中的实时时钟电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更换AFF C30 或AFF C60 存储系统中的实时时钟 (RTC) 电池（通常称为纽扣电池），以确保依赖于准确时间同步的服务和应用程序保持运行。

.开始之前
存储系统中的所有其他组件都必须正常运行。如果未能正常运行，请在继续此操作步骤之前联系 https://mysupport.netapp.com/site/global/dashboard["NetApp 支持"]。
...
+
您可以通过输入 `system location-led _off_`命令将其关闭。如果您不确定 LED 指示灯是否亮起，请输入 `system location-led show`命令检查其状态。
...
NOTE: 更换 RTC 电池、插入控制器并开机后，在第一次 BIOS 重置期间，您可能会看到以下错误消息：
`RTC date/time error. Reset date/time to default`
`RTC power failure error`这些消息属于预期情况，您可以继续执行此操作步骤。
...
NOTE: 如果存储系统停留在启动菜单处、请选择选项 `Reboot node`并在出现提示时响应_y_、然后按_Ctrl-C_启动到Loader。

. 在受损控制器上，在 LOADER 提示符下检查时间和日期：
+
`cluster date show`
...
. 在Loader提示符处、输入 `bye`以重新初始化I/O模块和其他组件、然后让控制器重新启动。
