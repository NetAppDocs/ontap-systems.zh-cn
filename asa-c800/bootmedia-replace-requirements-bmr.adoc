...
summary: 在更换 ASA C800 中的启动介质之前，请确保满足成功更换的必要要求。这包括验证您是否有正确的替换启动介质，确认受损控制器上的 e0S 或 e0M 扳手端口没有故障，并确定是否已启用板载密钥管理器或外部密钥管理器。 
---
= 查看 ASA C800 系统中自动启动介质恢复的要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换ASA C800中的启动介质之前，请确保满足成功更换的必要要求。这包括验证您是否拥有正确的替换启动介质、确认受损控制器上的 e0S（e0M 扳手）端口没有故障，以及确定是否启用了板载密钥管理器 (OKM) 或外部密钥管理器 (EKM)。
