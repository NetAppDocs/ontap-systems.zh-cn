...
summary: 当平台向系统控制台记录指示驱动器发生故障的警告消息时，请更换 ASA C800 系统中的驱动器。此外，操作员显示面板上的故障 LED 和故障驱动器上的故障 LED 都将点亮。 
---
= 更换 ASA C800 系统中的驱动器
:allow-uri-read: 
:icons: font
...
[role="lead"]
当ASA C800系统中的硬盘发生故障或需要升级时，请更换该硬盘。更换过程包括识别故障驱动器、安全移除它以及安装新驱动器以确保持续的数据访问和系统性能。
