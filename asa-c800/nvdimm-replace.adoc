...
summary: 当系统注册闪存寿命即将结束或已识别的 NVDIMM 不正常时，必须更换控制器模块中的 NVDIMM。否则会导致系统崩溃。 
---
= 更换 ASA C800 系统中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当ASA C800系统中检测到闪存寿命即将结束或检测到已识别的 NVDIMM 内存条状况不佳时，请更换 NVDIMM 内存条；否则会导致系统崩溃。

.开始之前
* 请确保您已收到从NetApp收到的替换 NVDIMM，并且该 NVDIMM 与您的ASA C800系统兼容。
