...
[role="lead"]
当 ASA C800 系统中的电池开始失去电量或出现故障时，请更换 NVDIMM 电池，因为它负责在停电期间保留关键系统数据。更换过程包括关闭受损控制器、卸下控制器模块、更换 NVDIMM 电池、重新安装控制器模块，以及将故障部件退回 NetApp。
...
更换 NVDIMM 电池的方法是：从控制器模块中取出故障电池，然后将新电池安装到控制器模块中。
