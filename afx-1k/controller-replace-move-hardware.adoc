...
summary: 当硬件故障需要更换 AFX 1K 存储系统中的控制器时。更换过程包括移除受损的控制器、将组件移至更换的控制器、安装更换的控制器以及重新启动它。 
---
= 更换控制器 - AFX 1K
:allow-uri-read: 
:icons: font
...
[role="lead"]
当硬件故障需要更换 AFX 1K 存储系统中的控制器时。更换过程包括移除受损的控制器、将组件移至更换的控制器、安装更换的控制器以及重新启动它。
...
. 将 DIMM 垂直插入插槽。
+
DIMM 紧紧地插入插槽，但您应该能够轻松插入它。如果不是，请将 DIMM 与插槽重新对齐并重新插入。
...
.下一步是什么？
更换损坏的 AFX 1K 控制器后，link:controller-replace-system-config-restore-and-verify.html["恢复系统设置"] 。
