...
summary: '在更换 FAS50 存储系统中的启动介质之前，请确认您符合成功更换的必要要求。这包括验证您是否具有正确的替换启动介质，确认受损控制器上的 e0M（扳手）端口是否正常工作，并确定是否已启用板载密钥管理器 (OKM) 或外部密钥管理器 (EKM)。' 
---
= 查看 FAS50 系统中自动启动介质恢复的要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换 FAS50 存储系统中的启动介质之前，请确认您符合成功更换的必要要求。这包括验证您是否具有正确的替换启动介质，确认受损控制器上的 e0M（扳手）端口是否正常工作，并确定是否已启用板载密钥管理器 (OKM) 或外部密钥管理器 (EKM)。
