...
summary: 将I/O模块添加到FAS50存储系统中、以增强网络连接并扩展系统处理数据流量的能力。 
---
= 将 I/O 模块添加到 FAS50 系统
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将I/O模块添加到FAS50存储系统中、以增强网络连接并扩展系统处理数据流量的能力。

当有可用插槽或所有插槽均已完全填充时、您可以将I/O模块添加到FAS50存储系统中(方法是移除现有I/O模块并在其位置安装新的I/O模块)。
