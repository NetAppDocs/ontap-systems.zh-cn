...
summary: 在 FAS50 存储系统中更换实时时钟电池（通常称为纽扣电池），以确保依靠精确时间同步的服务和应用程序保持运行。 
---
= 更换 FAS50 系统中的实时时钟电池
:allow-uri-read: 
:icons: font
...
[role="lead"]
更换 FAS50 存储系统中的实时时钟 (RTC) 电池（通常称为纽扣电池），以确保依赖于准确时间同步的服务和应用程序保持运行。
...
NOTE: 更换 RTC 电池、插入控制器并在第一次 BIOS 重置时打开电源后，您将看到以下错误消息：
