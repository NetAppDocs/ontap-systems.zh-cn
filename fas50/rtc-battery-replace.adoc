...
summary: '更换FAS50系统中的实时时钟(Real-time clock、RTC)电池(通常称为币形电池)、以确保依赖于准确时间同步的服务和应用程序保持正常运行。' 
---
= 更换实时时钟电池- FAS50
:allow-uri-read: 
:icons: font
...
[role="lead"]
更换FAS50系统中的实时时钟(Real-time clock、RTC)电池(通常称为币形电池)、以确保依赖于准确时间同步的服务和应用程序保持正常运行。
