...
summary: 将存储资源的控制权交还给替换控制器，以便您的 FAS50 系统可以恢复正常运行。恢复过程根据系统使用的加密类型而有所不同，包括无加密、板载密钥管理器 OKM 加密或外部密钥管理器 EKM 加密。 
---
= 归还 FAS50 系统中的控制器以恢复正常运行
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将存储资源的控制权归还给替代控制器、以便FAS50系统可以恢复正常运行。交还过程因系统使用的加密类型而异：无加密、板载密钥管理器(OKM)加密或外部密钥管理器(EKM)加密。
