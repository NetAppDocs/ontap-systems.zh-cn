...
summary: 在关闭受损节点并检查板载加密密钥的状态之前，必须检查受损节点的状态，禁用自动返还，并检查系统正在运行的ONTAP版本。FAS9000系统仅支持手动启动介质恢复程序。 
---
= 检查加密密钥支持和状态
:allow-uri-read: 
:icons: font
...
[role="lead"]
为确保存储系统的数据安全，您需要验证启动介质上的加密密钥支持情况和状态。检查您的ONTAP版本是否支持NetApp卷加密 (NVE)，并在关闭控制器之前检查密钥管理器是否处于活动状态。FAS9000系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。
