...
[role="lead"]
这些详细的分步说明将指导您完成 FAS9000 系统的安装，包括机架安装、布线到初始设置。如果您需要比快速步骤更深入的安装指导，请使用这些步骤。
...
+
image::../media/drw_9000_lifting_icon.png[起重重量小心标签]
...
. 连接缆线管理设备（如图所示）。
+
image::../media/drw_9000_cable_management_arms.png[连接缆线管理臂]
...
+
image::../media/drw_9000_TNSC_composite_cabling.png[双节点无交换机集群复合布线图]
...
+
image:../media/drw_9000_switched_cluster_cabling.png["交换集群复合布线图"]
...
您可以将新系统连接到 DS212C、DS224C 或 NS224 机架，具体取决于它是 AFF 还是 FAS 系统。
...
=== 选项2：使用缆线将控制器连接到仅运行ONTAP 9.8及更高版本的ASA AFF A700系统中的单个NS224驱动器架

必须将每个控制器连接到运行 ONTAP 9.8 或更高版本的 AFF A700 或 ASA AFF A700 系统上的 NS224 驱动器架上的 NSM 模块。

* 此任务仅适用于运行ONTAP 9.8或更高版本的 适用场景 ASA AFF A700。
* 系统必须至少在每个控制器的插槽 3 或 7 中安装一个 X91148A 模块。动画或插图显示此模块安装在插槽 3 和 7 中。
...
=== 选项3：使用缆线将控制器连接到仅运行ONTAP 9.8及更高版本的ASA AFF A700系统中的两个NS224驱动器架

必须将每个控制器连接到运行 ONTAP 9.8 或更高版本的 AFF A700 或 ASA AFF A700 系统上的 NS224 驱动器盘架上的 NSM 模块。
...
+
如果系统具有 NS224 驱动器盘架，则盘架将预设为盘架 ID 00 和 01。如果要更改盘架 ID，则必须创建一个工具以插入按钮所在的孔中。
...
+
.. 打开文件资源管理器。
.. 在左侧窗格中选择*网络*，右键单击*网络*，然后选择*刷新*。
.. 双击 ONTAP 图标并接受屏幕上显示的任何证书。
+

NOTE: 显示的值是目标节点的系统序列号。
