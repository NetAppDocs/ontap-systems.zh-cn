...
---
= 更换 NVRAM 模块和 / 或 NVRAM DIMM — AFF A700 和 FAS9000
:icons: font
:imagesdir: ../media/
...
[role="lead"]
根据您的配置，您必须验证是否已将磁盘重新分配给新控制器模块，或者手动重新分配磁盘。如果存储系统位于 HA 对中，则在操作步骤末尾发生交还时，新控制器模块的系统 ID 会自动分配给磁盘。在独立系统中，必须手动将 ID 重新分配给磁盘。
...
=== 在双节点 MetroCluster 配置中的系统上手动重新分配系统 ID
