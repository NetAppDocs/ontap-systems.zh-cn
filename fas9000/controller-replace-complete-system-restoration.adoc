...
summary: 要完成更换过程并将系统还原到完全运行状态，您必须重新连接存储，在必要时还原 NetApp 存储加密配置，并为新控制器安装许可证。 
---
= 完成 FAS9000 系统的控制器更换
:allow-uri-read: 
:icons: font
...
[role="lead"]
要完成 FAS9000 系统的控制器更换，请重新连接存储电缆，根据需要恢复 NetApp Storage Encryption 配置，并为新控制器安装许可证。
...
== 第 3 步（仅限 MetroCluster）：在双节点 MetroCluster 配置中切换回聚合
