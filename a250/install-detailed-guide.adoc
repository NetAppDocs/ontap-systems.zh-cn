...
[role="lead"]
使用这些详细的分步说明安装 AFF A250 存储系统，从准备站点到启动 ONTAP 集群。
...
.步骤
. 使用 25 GbE 集群互连电缆将集群互连端口 e0c 连接到 e0c，e0d 连接到 e0d。
+
...
NOTE: link:https://hwu.netapp.com["NetApp Hardware Universe"^]主机网卡（Fibre Channel 或 25 GbE）的插槽优先级为插槽 2。但是，如果您同时拥有这两个卡，则 Fibre Channel 卡位于插槽 2 中，25 GbE 卡位于插槽 1 中（如下面的选项中所示）。如果您有外部机架，则存储卡位于插槽 1 中，这是唯一受支持的机架插槽。
...
--
.选项 2：连接到 25 GbE 数据或主机网络
--
控制器上的 25 GbE 端口连接到 25 GbE 数据或主机网络交换机。
