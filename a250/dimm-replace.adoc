...
summary: 当系统遇到基于运行状况监视器警报的过多 CECC 错误或阻止存储系统启动 ONTAP 的不可纠正 ECC 错误时，请更换 AFF A250 控制器中的 DIMM。 
---
= 更换 AFF A250 系统中的 DIMM 以修复内存错误
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当存储系统遇到过多的可更正 (CECC) 或不可更正的内存错误时，请更换 AFF A250 系统中的 DIMM。此类错误可能会阻止存储系统启动 ONTAP。
