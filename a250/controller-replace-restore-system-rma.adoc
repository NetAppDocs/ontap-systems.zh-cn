...
summary: 要完成 AFF A250 系统的控制器更换，如有必要，请恢复 NetApp 存储加密配置，安装新控制器的许可证，并将故障部件退回 NetApp。 
---
= 完成 AFF A250 系统的控制器更换
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要完成 AFF A250 系统的控制器更换，请恢复 NetApp 存储加密配置（如有必要），确认逻辑接口（LIF）正在向其主端口报告，执行集群运行状况检查，并将故障部件返回 NetApp。
