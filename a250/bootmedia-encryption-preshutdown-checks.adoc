...
---
= 使用手动启动恢复过程检查 AFF A250 系统中启动介质上的加密支持
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
为了确保 AFF A250 存储系统的数据安全，请验证启动介质上的加密密钥支持和状态。检查您的 ONTAP 版本是否支持 NetApp 卷加密 (NVE)。在关闭控制器之前，请检查密钥管理器是否处于活动状态。
...
.下一步是什么？
检查启动介质上的加密密钥支持和状态后，link:bootmedia-shutdown.html["关闭控制器"]。
