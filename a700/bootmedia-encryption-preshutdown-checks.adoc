...
summary: 在关闭受损节点并检查板载加密密钥的状态之前，必须检查受损节点的状态，禁用自动返还，并检查系统正在运行的ONTAP版本。AFF A700系统仅支持手动启动介质恢复程序。 
---
= 检查 AFF A700 系统中启动介质上的加密支持
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
为确保存储系统的数据安全，您需要验证启动介质上的加密密钥支持情况和状态。AFF A700系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。
