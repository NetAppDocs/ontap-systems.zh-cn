...
summary: 在AFF A700系统上，从 USB 驱动器启动ONTAP恢复映像，以恢复启动介质。AFF A700系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 在 AFF A700 系统上启动恢复映像
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在AFF A700系统上，从 USB 驱动器启动ONTAP恢复映像，以恢复启动介质。该过程包括从 USB 闪存驱动器启动、恢复文件系统、验证环境变量以及使控制器恢复正常运行，具体步骤取决于系统是否处于双节点MetroCluster配置中。
