...
[role="lead"]
要更换 NVDIMM ，您必须使用风管顶部的 FRU 映射在控制器模块中找到它，或者使用插槽 1 提升板顶部的 FRU 映射查找警示 LED 。
...
您可以使用以下动画，绘图或写入步骤来更换 NVDIMM 。


NOTE: 此动画显示了没有 DIMM 的插槽中的空插槽。这些空插槽中填有空格。

https://netapp.hosted.panopto.com/Panopto/Pages/embed.aspx?id=e0afec49-0953-4dcc-b9d0-aadb01578e1b["更换 NVDIMM"]
...
. 交还节点的存储，使节点恢复正常运行： `storage failover giveback -ofnode ` impaired_node_name_``
