...
.步骤
. 重新对系统进行布线。
. 使用验证布线是否正确 https://["Active IQ Config Advisor"]。
+
...
+
http://["查找适用于您的 ONTAP 9 版本的《高可用性配置指南》"]
...
.. 运行 Config Advisor 。转到 NetApp 支持站点上的 Config Advisor 页面，网址为 http://["support.netapp.com/NOW/download/tools/config_advisor/"]。
+
