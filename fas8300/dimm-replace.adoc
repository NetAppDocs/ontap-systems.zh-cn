...
summary: 当存储系统遇到过多的可更正错误更正代码或不可更正错误等错误时，必须更换控制器中的 DIMM，这些错误通常是由单个 DIMM 故障导致存储系统无法启动 ONTAP。 
---
...
[role="lead"]
如果存储系统遇到错误，例如基于运行状况监视器警报的 CECC（可纠正错误更正代码）过多错误或不可纠正的 ECC 错误，则必须更换控制器中的 DIMM。这些错误通常是由单个 DIMM 故障引起的，会导致存储系统无法启动 ONTAP。
