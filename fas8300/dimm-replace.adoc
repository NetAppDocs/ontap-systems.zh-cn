...
summary: 当存储系统遇到过多的可纠正 ECC 内存错误或不可纠正 ECC 错误等错误时，必须更换控制器中的 DIMM，这些错误通常是由单个 DIMM 故障导致存储系统无法启动 ONTAP 所引起的。 
---
= 更换 FAS8300 或 FAS8700 系统中的 DIMM 以修复内存错误
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果存储系统遇到错误，例如基于运行状况监视器警报的可纠正 ECC 内存错误过多，或出现不可纠正的 ECC 错误，则必须更换控制器中的 DIMM。这些错误通常是由单个 DIMM 故障引起的，该故障会导致存储系统无法启动 ONTAP。

系统中的所有其他组件必须正常工作。如果组件无法正常工作，您必须联系技术支持。
...
系统中 DIMM 的数量和位置取决于系统的型号。有关详细信息，请参见风道上的 FRU 图。
...
+
DIMM 可以紧紧地插入插槽，但应易于插入。如果没有，请将 DIMM 与插槽重新对齐并重新插入。
...
. 完成控制器模块的安装：
+
.. 使用锁定闩锁将控制器模块牢固地推入机箱，直到锁定闩锁开始上升。
+
