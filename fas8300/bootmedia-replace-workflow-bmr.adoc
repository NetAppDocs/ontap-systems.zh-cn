...
summary: 开始更换FAS8300或FAS8700存储系统中的启动介质，方法是查看更换要求、关闭控制器、更换启动介质、恢复启动介质上的映像并验证系统功能。 
---
= 使用自动启动恢复过程的 FAS8300 或 FAS8700 系统的启动介质更换工作流程
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用自动引导恢复过程，开始更换 FAS8300 或 FAS8700 存储系统中的引导介质。系统会自动识别并选择适当的引导菜单选项，然后使用配对节点上的引导映像在替换引导介质上重新安装 ONTAP。要完成更换，请查看要求、关闭控制器、更换引导介质、允许系统还原映像，并验证系统功能。
