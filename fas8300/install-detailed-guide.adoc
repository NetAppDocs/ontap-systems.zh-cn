...
---
= 使用详细的分步说明安装 FAS8300 或 FAS8700 系统
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
这些详细的分步说明将指导您安装 FAS8300 或 FAS8700 系统，从机架安装和布线到初始设置。如果您需要比快速步骤提供的更深入的安装指导，请使用这些步骤。
...
NOTE: 如果看不到卡上的端口标签，请检查卡的安装方向（PCIe 连接器插槽位于 A400 和 FAS8300/8700 卡插槽的左侧），然后按部件号在 https://hwu.netapp.com["NetApp Hardware Universe"^]中查找该卡，以获取显示端口标签的挡板图形。可以使用 `sysconfig -a`命令或在系统装箱单上找到卡的部件号。
...
+
image::../media/drw_fas8300_switched_network_cabling.png[交换集群网络布线图]
...
+
image::../media/drw_fas8300_three-ds224c_shelves.png[三张DS224C磁盘架布线图]
