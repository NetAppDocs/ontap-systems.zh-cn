...
summary: 当您的系统注册闪存寿命即将结束或所识别的 NVDIMM 总体上不健康时，您必须更换控制器模块中的 NVDIMM。 
---
= 更换 FAS8300 或 FAS8700 系统中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
系统中的所有其他组件必须正常工作。如果不是，则必须联系技术支持。
...
要更换 NVDIMM，必须使用插槽 1 提升板顶端的 FRU 映射在控制器模块中找到它。

* 在暂停系统后，在转存内容时， NVDIMM LED 会闪烁。目标值完成后，此 LED 将熄灭。
* 虽然 NVDIMM 的内容已加密，但最佳实践是在更换 NVDIMM 之前擦除其内容。有关详细信息，请参见 NetApp 支持站点上的 https://mysupport.netapp.com/info/web/ECMP1132988.html["易失性声明"]。
+
...
. 将 NVDIMM 垂直插入插槽。
+
NVDIMM 紧贴插槽，但应易于插入。如果没有，请将 NVDIMM 与插槽重新对齐并重新插入。
...
. 交还控制器的存储，使其恢复正常运行： `storage failover giveback -ofnode ` impaired_node_name_``
