...
summary: 当系统注册单个 AutoSupport 消息指示模块已离线时，必须更换控制器模块中的缓存模块。 
---
= 更换 FAS8300 或 FAS8700 系统中的缓存模块
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
NOTE: VER2 控制器模块在 FAS8300 中只有一个缓存模块插槽。FAS8700 没有 VER2 控制器模块。缓存模块功能不受插槽移除的影响。
...
NOTE: 插槽 6 仅适用于 FAS8300 VER2 控制器。

您的存储系统必须满足特定条件，具体取决于您的情况：

* 它必须具有与要安装的缓存模块对应的操作系统。
* 它必须支持缓存容量。
* 虽然缓存模块的内容已加密，但最佳实践是在更换该模块之前擦除其内容。有关详细信息，请参见 NetApp 支持站点上适用于您系统的 https://mysupport.netapp.com/info/web/ECMP1132988.html["易失性声明"]。
+

NOTE: 您必须登录到 NetApp 支持站点，才能显示系统的 _statement of volatil性 _ 。

* 存储系统中的所有其他组件都必须正常运行。如果未能正常运行，请联系技术支持。
...
. 使用控制器模块上的 FRU 映射找到出现故障的缓存模块，然后将其移除：
+
根据您的配置，控制器模块中可能有零个、一个或两个缓存模块。使用控制器模块内的 FRU 映射来帮助定位缓存模块。
