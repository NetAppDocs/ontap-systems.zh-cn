...
---
= 卸下控制器模块，更换启动介质并将启动映像传输到启动介质— FAS8300 和 FAS8700
:icons: font
:imagesdir: ../media/
...
您可以使用以下动画或写入的步骤来更换启动介质。

https://netapp.hosted.panopto.com/Panopto/Pages/embed.aspx?id=e0825a97-c57d-47d7-b87d-aad9012efa12["更换启动介质"]
...
.. 尽管环境变量和 bootarg 已保留，但您应使用 `printenv bootarg name` 命令检查是否已为您的系统类型和配置正确设置所有必需的启动环境变量和 bootarg ，并使用 `setenv _variable-name_ <value>` 命令更正任何错误。
+
... 检查启动环境变量：
+
...
**** `kmip.init.interface _ < 值 > _`
**** `kmip.init.ipaddr _ < 值 > _`
**** `kmip.init.netmask _ < 值 > _`
**** `kmip.init.gateway _ < 值 > _`


... 如果启用了板载密钥管理器，请检查 `kenv` ASUP 输出中列出的 bootarg 值：
+
**** `bootarg.storageencryption.support _<value>_`
**** `bootarg.keymanager.support _ < 值 >` _
**** `bootarg.onbox_keymanager _<value>_`
