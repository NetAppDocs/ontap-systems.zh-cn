...
summary: 如果存储系统向系统控制台记录一条警告消息、指出某个驱动器出现故障、则必须更换该驱动器。In addition, both the fault LED on the operator display panel and the fault LED on the failed drive are illuminated. 
---
= 更换SSD驱动器—AFF C80
:allow-uri-read: 
:icons: font
