...
在更换控制器模块或内部组件之前，请先将控制器模块从机箱中取出。
...
.下一步是什么？
更换损坏的AFF A70或AFF A90控制器后，您需要：link:controller-replace-system-config-restore-and-verify.html["还原系统配置"] 。
