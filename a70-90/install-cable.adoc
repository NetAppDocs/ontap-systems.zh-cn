...
summary: 为AFF A70或AFF A90存储系统安装机架硬件后、请安装控制器的网络缆线、并在控制器和存储架之间连接缆线。 
---
= 连接 AFF A70 或 AFF A90 硬件以实现网络和存储连接
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
* AFF A70和AFF A90控制器上的I/O插槽编号为1到11。
+
...
--
使用 100 GbE 电缆将控制器 A 和 B 的 e1a 和 e7a 端口连接到集群网络交换机 A 和 B。
...
NOTE: 不要将端口e1b和e7b用于主机网络连接。请使用单独的主机卡。
...
以下布线步骤演示了如何将控制器连接到存储架。从以下与您的设置相匹配的布线选项中选择一项。

有关存储系统和所有布线选项支持的最大磁盘架数量，请参见link:https://hwu.netapp.com["NetApp Hardware Universe"^]。

.关于此任务
AFF A70 和 AFF A90 存储系统支持配备 NSM100 或 NSM100B 模块的 NS224 盘架。这些模块之间的主要区别是：
...
.. 将端口 e11b 连接到磁盘架 2，NSM A 端口 e0b。
+
...
====
.下一步是什么？
在为 AFF A70 或 AFF A90 系统连接硬件电缆后，您 link:install-power-hardware.html["打开AFF A70或AFF A90存储系统的电源"]。
