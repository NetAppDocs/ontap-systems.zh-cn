...
summary: 当非易失性内存出现故障或需要升级时，请更换AFF A70或AFF A90系统中的NVRAM。 
---
= 更换 AFF A70 或 AFF A90 系统中的 NVRAM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当非易失性存储器出现故障或需要升级时，请更换AFF A70或AFF A90系统中的NVRAM 。更换过程包括关闭受损的控制器、将控制器拉出几英寸以完全切断电源、更换NVRAM模块或NVRAM DIMM、重新分配磁盘，以及将故障部件返回给NetApp。
...
WARNING: NVRAM 模块不可热插拔。在更换之前，请重新拔插驱动器，关闭节点，拔下控制器电源，然后将控制器从机箱中拉出几英寸。
...
----
+
*** 如果命令返回 `There are no entries matching your query.`继续<<Go-to-the-rear,进入下一步>>。
*** 如果该命令返回任何其他结果，请从两个控制器收集AutoSupport数据，并联系NetApp支持部门以获取进一步帮助。
+
[source, cli]
----
