...
summary: 为了确保 AFF A70 或 AFF A90 存储系统的数据安全，需要验证启动介质上的加密密钥支持和状态。检查 ONTAP 版本是否支持 NetApp 卷加密，并在关闭控制器之前检查密钥管理器是否处于活动状态。 
---
= 使用手动启动恢复过程检查 AFF A70 或 AFF A90 系统中启动介质上的加密支持
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
为了确保 AFF A70 或 AFF A90 存储系统的数据安全，需要验证启动介质上的加密密钥支持和状态。检查您的 ONTAP 版本是否支持 NetApp 卷加密 (NVE)。在关闭控制器之前，请检查密钥管理器是否处于活动状态。
