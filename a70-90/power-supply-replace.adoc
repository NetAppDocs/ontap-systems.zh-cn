...
summary: 当 AFF A70 或 AFF A90 系统中的交流或直流电源装置发生故障或出现问题时，请进行更换，以确保系统持续获得稳定运行所需的电源。更换过程包括从电源断开有故障的 PSU，拔下电源线，更换有故障的 PSU，然后将其重新连接到电源。 
---
= 热插拔 AFF A70 或 AFF A90 系统中的电源单元
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A70或AFF A90系统中的交流或直流电源设备(PSU)出现故障或出现故障时、请予以更换、以确保系统继续获得稳定运行所需的电源。更换过程包括断开故障PSU与电源的连接、拔下电源线、更换故障PSU、然后将其重新连接到电源。

电源是冗余的，并且可热插拔。您不必关闭控制器来更换 PSU。

.关于此任务
此程序用于一次更换一个 PSU。根据您的 PSU 类型使用适当的程序：AC 或 DC。
