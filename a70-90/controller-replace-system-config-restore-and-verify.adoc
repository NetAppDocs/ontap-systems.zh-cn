...
summary: 确认控制器的HA配置在AFF A70或AFF A90存储系统中处于活动状态且正常运行、并确认系统的适配器列出了磁盘的所有路径。 
---
= 还原并验证 AFF A70 或 AFF A90 系统中的控制器配置
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
确认控制器的HA配置在AFF A70或AFF A90存储系统中处于活动状态且正常运行、并确认系统的适配器列出了磁盘的所有路径。
...
.下一步是什么？
恢复并验证AFF A70或AFF A90系统的系统配置后，您需要link:controller-replace-recable-reassign-disks.html["交还控制器"]。
