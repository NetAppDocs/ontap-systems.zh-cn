...
. 选择列出的 ONTAP 图标以发现：
+
image::../media/drw_autodiscovery_controler_select.svg[如何使用Windows文件资源管理器发现控制器]
...
.开始之前
确保您的笔记本电脑已连接到串行控制台端口、并且控制器已打开电源。有关说明、请参见。 link:install-power-hardware.html#step-2-power-on-the-controllers["Power on the controllers"]
...
NOTE: 初始化集群后、下载并运行  https://mysupport.netapp.com/site/tools/tool-eula/activeiq-configadvisor["Active IQ Config Advisor"] 以确认设置。
