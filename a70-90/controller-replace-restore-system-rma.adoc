...
summary: 验证逻辑接口是否正在向其主端口报告、执行集群运行状况检查、并将故障部件返回给NetApp以完成AFF A70和AFF A90控制器更换工作流中的最后一步。 
---
...
[role="lead"]
验证逻辑接口是否正在向其主端口报告、执行集群运行状况检查、并将故障部件返回给NetApp以完成AFF A70和AFF A90控制器更换工作流中的最后一步。
