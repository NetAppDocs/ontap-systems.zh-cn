...
系统提示符或密码提示符（输入系统密码）
 a| 
接管或暂停受损节点： `storage failover takeover -ofnode _impaired_node_name_`
...
[role="lead"]
要更换 DIMM ，您必须使用控制器模块内部的 DIMM 映射在控制器模块中找到该 DIMM ，或者使用 DIMM 旁边的 LED 找到该 DIMM ，然后按照特定步骤顺序进行更换。
