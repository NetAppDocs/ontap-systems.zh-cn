...
summary: 使用辅助启动介质或 USB 闪存驱动器，将启动映像传输到AFF A700s系统上的替换启动介质。AFF A700s系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 使用 USB 闪存驱动器将启动映像传输至 AFF A700s 系统中的启动介质
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
使用辅助启动介质（主要方法）或 USB 闪存驱动器（如果辅助启动介质恢复失败或 `image.tgz`文件丢失）将启动映像传输到 AFF A700s 系统上的替换启动介质。
...
== 选项 1：使用备份恢复从第二个启动介质传输系统映像

您可以使用控制器模块中第二个启动介质上的映像将系统映像安装到替换启动介质。这是控制器模块中具有两个启动介质的系统的主要方法。

辅助启动介质上的映像必须包含  `image.tgz` 文件，并且不得报告故障。如果  `image.tgz` 文件丢失或启动介质报告故障，则无法使用此过程。请改用 <<方案二：使用U盘传输启动镜像>>。
...
仅当辅助启动介质还原失败或在辅助启动介质上未找到  `image.tgz` 文件时，才应使用此过程。

* 您必须具有一个已格式化为 32 位的 USB 闪存驱动器，并且容量至少为 4 GB 。
* 与受损控制器正在运行的相同映像版本的 ONTAP 副本。您可以从 NetApp 支持站点的"下载"部分下载相应的映像。
+
