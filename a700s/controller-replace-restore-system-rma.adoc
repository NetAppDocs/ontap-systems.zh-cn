...
summary: 要将 AFF A700s 系统恢复到完全运行状态，请在必要时恢复 NetApp 存储加密配置，安装新控制器的许可证，并将故障部件返回 NetApp。 
---
= 完成 AFF A700s 系统的控制器更换
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
通过恢复 NetApp 存储加密配置（如有必要）、为新控制器安装许可证并将故障部件退回 NetApp，完成 AFF A700s 系统的控制器更换。
