...
summary: '要完成AFF A20、 AFF A30 或AFF A50 存储系统的控制器更换，请首先恢复NetApp存储加密配置（如有必要）并在新控制器上安装所需的许可证。接下来，确认逻辑接口 (LIF) 正在向其主端口报告并执行集群健康检查。最后，注册新控制器的序列号，然后将发生故障的部件退回给NetApp。' 
---
