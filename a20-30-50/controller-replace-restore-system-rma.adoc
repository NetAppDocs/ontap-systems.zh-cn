...
summary: 要完成 AFF A20、AFF A30 或 AFF A50 存储系统的控制器更换，请恢复 NetApp Storage Encryption 配置并在新控制器上安装所需的许可证。确认逻辑接口正在向其主端口报告并执行集群运行状况检查。注册新控制器的序列号，并将故障部件退回 NetApp。 
---
= 完成 AFF A20、AFF A30 或 AFF A50 系统的控制器更换
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要完成AFF A20、 AFF A30 或AFF A50 存储系统的控制器更换，请首先恢复NetApp存储加密配置（如有必要）并在新控制器上安装所需的许可证。接下来，确认逻辑接口 (LIF) 正在向其主端口报告并执行集群健康检查。最后，注册新控制器的序列号，然后将发生故障的部件退回给NetApp。
