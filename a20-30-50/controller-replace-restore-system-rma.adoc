...
summary: 要将AFF A20、AFF A30或AFF A50系统还原到完全运行状态、请在新控制器上安装所需的许可证、确认逻辑接口是否向其主端口报告、然后执行集群运行状况检查。最后、注册新控制器的序列号、然后将故障部件退回给NetApp。 
---
...
[role="lead"]
要完成AFF A20、AFF A30或AFF A50系统的控制器更换、请先还原NetApp存储加密配置(如有必要)、并在新控制器上安装所需的许可证。接下来、确认逻辑接口(Logical Interface、Logical Interface、Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称为Logical Interface、简称最后、注册新控制器的序列号、然后将故障部件退回给NetApp。
