...
---
= 将 I/O 模块添加到 AFF A20、AFF A30 或 AFF A50 系统
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
向您的AFF A20、 AFF A30 或AFF A50 存储系统添加 I/O 模块，以增强网络连接并扩展系统处理数据流量的能力。

添加 I/O 模块涉及关闭一个控制器、添加模块并重新启动，然后对另一个控制器重复这些步骤。

.关于此任务
* 当有可用插槽或所有插槽都已完全填充时，您可以将 I/O 模块添加到存储系统。
...
+
您可以通过输入 `system location-led _off_`命令将其关闭。如果您不确定 LED 指示灯是否亮起，可以通过输入 `system location-led show`命令来检查其状态。
