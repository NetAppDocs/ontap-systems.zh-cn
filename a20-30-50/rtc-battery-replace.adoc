...
summary: 在您的 AFF A20、AFF A30 或 AFF A50 存储系统中更换实时时钟电池（通常称为纽扣电池），以确保依赖准确时间同步的服务和应用程序保持正常运行。 
---
= 更换 AFF A20、AFF A30 或 AFF A50 系统中的实时时钟电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更换AFF A20、 AFF A30 或AFF A50 存储系统中的实时时钟 (RTC) 电池（通常称为纽扣电池），以确保依赖于准确时间同步的服务和应用程序保持运行。

.开始之前
存储系统中的所有其他组件都必须正常运行。如果没有，请在继续此程序之前联系 https://mysupport.netapp.com/site/global/dashboard["NetApp 支持"]。
...
NOTE: 更换 RTC 电池、插入控制器并打开系统电源以进行首次 BIOS 重置后，您将看到以下错误消息：
`RTC date/time error. Reset date/time to default`
`RTC power failure error`这些消息属于预期情况，您可以继续执行此过程。
...
NOTE: 如果存储系统在引导菜单处停止，请选择 `Reboot node`对应的选项，并在出现提示时输入 _y_，然后按 _Ctrl-C_ 引导至 LOADER。
...
. 在 LOADER 提示符处，输入 `bye`以重新初始化 I/O 模块和其他组件，并重新启动控制器。
