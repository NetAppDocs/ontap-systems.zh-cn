...
summary: 当驱动器出现故障或需要升级时，在 AFF A20、AFF A30 或 AFF A50 存储系统中热插拔驱动器。更换过程包括识别故障驱动器、安全地将其移除，以及安装新驱动器，以确保持续的数据访问和系统性能。 
---
= 热插拔 AFF A20、AFF A30 或 AFF A50 系统中的驱动器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当驱动器出现故障或需要升级时，在 AFF A20、AFF A30 或 AFF A50 存储系统中热插拔驱动器。更换过程包括识别故障驱动器、安全地将其移除，以及安装新驱动器，以确保持续的数据访问和系统性能。
...
. 如果要更换另一块硬盘，请重复上述步骤。
. 重新安装存储系统正面的挡板。
. 如果在此过程的早期阶段禁用了自动驱动器分配，请手动分配驱动器所有权，然后根据需要重新启用自动驱动器分配：
+
.. 显示所有未拥有的驱动器：
+
...
+
请联系技术支持。 https://mysupport.netapp.com/site/global/dashboard["NetApp 支持"]如果您需要RMA编号或更换流程方面的其他帮助，请致电888-463-8277（北美）、00-800-44-638277（欧洲）或+800-800-80-800（亚太地区）。//2025-11-17 ontap-systems-internal/issues/1391
