...
summary: 关闭 AFF A900 存储系统中的控制器，以防止数据丢失，并在手动启动介质恢复过程中保持系统稳定性。 
---
= 在手动启动恢复过程中关闭 AFF A900 系统中的控制器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
关闭 AFF A900 存储系统中的控制器，以防止数据丢失，并在手动启动介质恢复过程中保持系统稳定性。
...
.大多数配置
--
完成 NVE 或 NSE 任务后，需要完成控制器的关闭。

如果您的存储系统运行的是ONTAP 9.17.1 或更高版本，请使用自动启动恢复过程。如果您的系统运行的是早期版本的ONTAP，则必须使用手动启动恢复过程。
