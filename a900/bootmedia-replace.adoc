...
---
= 使用手动启动恢复过程更换 AFF A900 系统中的启动介质
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
AFF A900 系统中的启动介质存储重要的固件和配置数据。更换过程包括卸下控制器模块、卸下受损的启动介质、安装更换用的启动介质，然后使用 USB 闪存驱动器将 ONTAP 映像传输到更换用的启动介质中。
...
.. 将MetroCluster端口设置为启动器： `ucadmin modify -m fc -t initiator adapter_name`
.. halt 返回维护模式： `halt`
