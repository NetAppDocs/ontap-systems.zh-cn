...
* For instructions about hot swapping the caching module, see link:../a900/caching_module_hot_swap.html["热插拔缓存模块"]。
...
[role="lead"]
在 <stdin> 中未解析的指令 - 包括：： .//_include/shutdown_any_mcc_supported 。
...
... 按下核心转储模块正面的 Terra cotta 释放按钮。
+

IMPORTANT: 请勿使用带编号和字母的 I/O 凸轮闩锁弹出核心转储模块。带编号和字母的 I/O 凸轮闩锁可弹出整个 NVRAM10 模块，而不是核心转储模块。
