...
summary: 当系统注册了越来越多的可更正的纠错代码或不可更正的 ECC 错误时，请更换 AFF A900 控制器模块中的 DIMM。未能更换有故障的 DIMM 可能会导致系统崩溃。 
---
= 更换 AFF A900 系统中的 DIMM 以修复内存错误
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当存储系统遇到过多的可纠正 (CECC) 或不可纠正的内存错误时，请更换 AFF A900 系统中的 DIMM。此类错误可能会阻止存储系统启动 ONTAP。
