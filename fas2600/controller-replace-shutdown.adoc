...
summary: 在更换 FAS2600 系统中受损的控制器之前，请先确定其状态，并在必要时将其接管，以便正常的控制器继续提供数据。 
---
= 在更换之前关闭 FAS2600 系统中受损的控制器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换之前，通过确定 FAS2600 系统中受损控制器的状态，并在必要时将其接管，以便正常控制器继续从受损控制器存储中提供数据，从而关闭受损控制器。
