...
summary: 当您的系统收到单个 AutoSupport 消息，提示模块已离线时，请更换 FAS2600 系统中的缓存模块。否则会导致性能下降。 
---
= 更换 FAS2600 系统中的缓存模块
:allow-uri-read: 
:icons: font
...
[role="lead"]
当系统注册单个 AutoSupport (ASUP) 消息表明该模块已离线时，请更换 FAS2600 系统中的缓存模块。否则会导致性能下降。

* 您必须将故障组件更换为从提供商处收到的替代 FRU 组件。


link:https://youtu.be/LaMzvnAwM_I["AFF FAS2600 缓存模块更换视频"^]
...
缓存模块在控制器标签上称为 M.2 PCIe 卡。找到控制器内的插槽，并按照此步骤顺序进行操作。
