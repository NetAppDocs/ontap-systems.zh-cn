...
summary: 当存储系统遇到基于运行状况监视器警报的过多可纠正 ECC 错误，或由单个 DIMM 故障导致的不可纠正 ECC 错误（阻止系统启动 ONTAP）时，请更换 FAS2600 系统中的 DIMM。 
---
= 更换 FAS2600 系统中的 DIMM 以修复内存错误
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当存储系统根据 Health Monitor 警报遇到过多的 CECC（可纠正错误更正代码）错误或不可纠正的 ECC 错误时，请更换 FAS2600 系统中的 DIMM，这些错误通常由单个 DIMM 故障引起，导致存储系统无法启动 ONTAP。

如果任何其他组件无法正常工作，请与技术支持联系。
