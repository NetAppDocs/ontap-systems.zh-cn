...
summary: 在 FAS2600 系统上，从 USB 驱动器启动ONTAP恢复映像，以恢复启动介质。FAS2600 系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 启动恢复映像— FAS2600
:allow-uri-read: 
:icons: font
...
[role="lead"]
在 FAS2600 系统上，从 USB 驱动器启动ONTAP恢复映像，以恢复启动介质。此过程包括从 USB 闪存驱动器启动、恢复文件系统、验证环境变量以及在更换启动介质后使控制器恢复正常运行。
