...
summary: 更换 ASA A1K 系统中的实时时钟电池（通常称为纽扣电池），以确保依靠精确时间同步的服务和应用程序保持运行。 
---
= 更换 ASA A1K 系统中的实时时钟电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更换ASA A1K系统中的实时时钟(RTC)电池(通常称为币形电池)、以确保依赖于准确时间同步的服务和应用程序保持正常运行。

.开始之前
* 请注意、您可以对系统支持的所有ONTAP版本使用此过程。
* 确保系统中的所有其它组件均正常工作；如果不能正常工作，则必须与联系 https://support.netapp.com["NetApp 支持"]。
