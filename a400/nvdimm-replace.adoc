...
summary: 当系统注册闪存寿命即将结束或识别的 NVDIMM 不正常时，请更换 AFF A400 控制器模块中的 NVDIMM。未能更换故障的 NVDIMM 可能会导致系统崩溃。 
---
= 更换 AFF A400 系统中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当您的系统检测到闪存寿命即将结束或 NVDIMM 运行不正常时，请更换 AFF A400 系统中的 NVDIMM。如果未能及时更换，可能会导致系统崩溃。
