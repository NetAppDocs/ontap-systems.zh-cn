...
summary: 在更换的启动介质上恢复加密。完成特定于启用了板载密钥管理器、NetApp 存储加密或 NetApp 卷加密的系统的步骤。 
---
= 在手动启动恢复过程后，在 AFF A400 系统中的启动介质上恢复加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 AFF A400 系统中的替换引导介质上恢复加密，以确保持续的数据保护。更换过程包括验证密钥可用性、重新应用加密设置以及确认对您的数据的安全访问。
