...
[role="lead"]
使用这些详细的分步说明安装 AFF A400 存储系统，从准备现场到启动 ONTAP 集群。
...
NOTE: 如果看不到卡上的端口标签，请检查卡的安装方向。PCIe 连接器插座位于 A400 和 FAS8300/8700 中卡插槽的左侧。然后按部件号在 https://hwu.netapp.com["NetApp Hardware Universe"^]中查找卡，以查看显示端口标签的挡板图形。您可以使用 `sysconfig -a`命令或在系统装箱单上找到卡的部件号。
...
+
.. 打开文件资源管理器。
.. 在左侧窗格中选择 *Network*，然后右键单击并选择 *refresh*。
