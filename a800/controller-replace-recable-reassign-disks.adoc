...
[role="lead"]
在更换控制器后重新为 AFF A800 系统布线，并将磁盘重新分配给更换用的控制器。如果您的系统处于 HA 对中，则交还过程会自动将系统 ID 分配给磁盘；确切的步骤取决于您的系统是未使用加密还是板载密钥管理器 (OKM) 加密。
...
如果存储系统处于 HA 对中，则在程序结束时发生交还时，新控制器模块的系统 ID 将自动分配给磁盘。您必须在启动 _replacement_ 控制器时确认系统 ID 更改，然后验证系统是否已应用此更改。

此步骤仅适用于在 HA 对中运行 ONTAP 的系统。
