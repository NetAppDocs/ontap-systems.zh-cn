...
summary: 在更换 AFF A800 系统中的启动介质之前，请验证您是否符合成功更换的必要要求。 
---
= 查看 AFF A800 系统中自动启动介质恢复的要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换 AFF A800 系统中的启动介质之前，请验证您是否符合成功更换的必要要求。这包括验证您是否有正确的替换启动介质，确认受损控制器上的 e0S（e0M 扳手）端口没有故障，并确定是否已启用 Onboard Key Manager（OKM）或 External Key Manager（EKM）。
