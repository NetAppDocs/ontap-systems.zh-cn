...
summary: 当系统注册闪存寿命即将结束或识别的 NVDIMM 不正常时，请更换 AFF A800 控制器模块中的 NVDIMM。未能更换故障的 NVDIMM 可能会导致系统崩溃。 
---
= 更换 AFF A800 系统中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当您的系统检测到闪存寿命即将结束或已识别的 NVDIMM 运行状态不正常时，请更换 AFF A800 系统中的 NVDIMM。如果未能更换故障的 NVDIMM，将导致系统崩溃。

.开始之前
* 请确保您已收到 NetApp 提供的替换 NVDIMM，并且该 NVDIMM 与您的 AFF A800 系统兼容。
* 确保存储系统中的所有其他组件都正常运行。如果没有，请联系 NetApp 支持部门。
...
使用风管顶部的 NVDIMM 定位标签找到控制器模块中的 NVDIMM，然后进行更换。
