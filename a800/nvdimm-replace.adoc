...
[role="lead"]
当您的AFF A800系统检测到闪存寿命即将结束或检测到已识别的 NVDIMM 内存条状况不佳时，请更换 NVDIMM 内存条；否则会导致系统崩溃。

.开始之前
* 请确保您已收到从NetApp收到的替换 NVDIMM，并且该 NVDIMM 与您的AFF A800系统兼容。
* 请确保存储系统中的所有其他组件均正常运行；如果出现问题，请联系NetApp支持。
