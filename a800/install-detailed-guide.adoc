...
[role="lead"]
从准备现场到启动 ONTAP 集群，请按照这些详细的分步说明安装 AFF A800 存储系统。
...
使用双节点无交换机集群方法或集群互连网络方法，您的平台集群需要进行布线。可选择布线连接到光纤通道或 iSCSI 主机网络或直连存储。此布线并非专用；您可以同时布线连接到主机网络和存储。
