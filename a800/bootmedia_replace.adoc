...
. Gently push the controller module all the way into the system until the controller module locking hooks begin to rise, firmly push on the locking hooks to finish seating the controller module, and then swing the locking hooks into the locked position over the pins on the controller module.+ 控制器一旦完全安装到机箱中，就会开始启动。
. Interrupt the boot process by pressing Ctrl-C to stop at the LOADER prompt.+ 如果未显示此消息，请按 Ctrl-C ，选择启动到维护模式的选项，然后暂停控制器以启动到加载程序。
