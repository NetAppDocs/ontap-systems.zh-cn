...
summary: 要将系统还原到完全运行状态，请在必要时还原 NetApp 存储加密配置，安装新控制器的许可证，并将出现故障的部件返回 NetApp。 
---
= 完成 ASA A250 系统的控制器更换
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要完成 ASA A250 系统的控制器更换以完全运行，您必须恢复 NetApp 存储加密配置（如有必要），并为新控制器安装许可证，然后将故障部件退回 NetApp，如套件附带的 RMA 说明中所述。
