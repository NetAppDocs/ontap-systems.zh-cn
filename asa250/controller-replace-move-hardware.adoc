...
[role="lead"]
当硬件故障需要时，请更换 ASA A250 系统中的控制器。更换过程包括移除受损的控制器，将组件移动到更换控制器，安装更换控制器并重新启动。
...
.下一步是什么？
更换损坏的 ASA A250 控制器后，需要link:controller-replace-system-config-restore-and-verify.html["恢复并验证系统配置"]。
