...
[role="lead"]
完成硬件更换并将 AFF C250 系统引导到维护模式后，需要验证更换控制器的低级系统配置，并根据需要重新配置系统设置。
...
.下一步是什么？
恢复并验证系统配置后，需要执行此操作link:controller-replace-recable-reassign-disks.html["重新连接系统并重新分配磁盘"]。
