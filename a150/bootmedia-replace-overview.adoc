...
summary: 启动介质存储系统启动时使用的主要和次要系统启动映像文件集。根据您的网络配置，您可以执行无中断或中断性更换。 
---
= 查看在 AFF A150 系统中更换启动介质的要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换 AFF A150 系统中的启动介质之前，请确认您符合成功更换的必要要求。这包括将 USB 闪存驱动器格式化为 FAT32，并为 ONTAP 恢复映像提供足够的存储空间，以及了解您执行的是无中断更换还是中断更换。
