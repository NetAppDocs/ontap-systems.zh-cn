...
+
.. 擦除缓存模块上的数据： `system controller flash-cache secure-erase run`
.. 验证是否已从缓存模块中擦除数据： `system controller flash-cache secure-erase show -node node_name`
+
