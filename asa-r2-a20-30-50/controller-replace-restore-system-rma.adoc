...
summary: '还原NetApp存储加密配置(如有必要)、为替代控制器安装许可证、验证逻辑接口(Logical Interface、ASA)是否向其主端口报告、执行集群运行状况检查、注册替代控制器的序列号、然后将故障部件返回给以完成ASA A20、ASA A30或NetApp A50控制器更换过程中的最后一步。' 
---
...
[role="lead"]
还原NetApp存储加密配置(如有必要)、为替代控制器安装许可证、验证逻辑接口(Logical Interface、ASA)是否向其主端口报告、执行集群运行状况检查、注册替代控制器的序列号、然后将故障部件返回给以完成ASA A20、ASA A30或NetApp A50控制器更换过程中的最后一步。
