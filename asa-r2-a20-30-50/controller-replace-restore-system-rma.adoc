...
summary: 要完成 ASA A20、ASA A30 或 ASA A50 存储系统的控制器更换，首先要恢复 NetApp 存储加密配置，然后在新控制器上安装所需的许可证。接下来，请确认逻辑接口正在向其主端口报告，并执行集群运行状况检查。最后，注册新控制器的序列号，然后将出现故障的部件返回 NetApp。 
---
= 完成 ASA A20、ASA A30 或 ASA A50 系统的控制器更换
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要完成ASA A20、 ASA A30 或ASA A50 存储系统的控制器更换，请先恢复NetApp存储加密配置（如有必要）并在新控制器上安装所需的许可证。接下来，确认逻辑接口 (LIF) 正在向其主端口报告并执行集群健康检查。最后，注册新控制器的序列号，然后将发生故障的部件退回给NetApp。
