...
====
* 如果您有 AFF A700 或 ASA A700HA 对，并且要热添加初始 NS224 驱动器架（ HA 对中不存在 NS224 驱动器架），则必须在每个控制器中安装一个核心转储模块（ X9170A ， NVMe 1 TB SSD ），以支持核心转储（存储核心文件）。
+
https://docs.netapp.com/platstor/topic/com.netapp.doc.hw-9000-nvmeflashcacherep/GUID-90B4E3EF-FBEF-4A97-9144-42B6400C16AA.html["更换缓存模块或添加 / 更换核心转储模块"]
...
=== 准备 AFF A700 ， ASA A700 ， ASA A800 ， AFF A800 ， AFF A400 ， 或 ASA A400HA 对以热添加第二个磁盘架

[role="lead"]
如果您使用的是 AFF A700 ， ASA A700 ， AFF A800 ， ASA A800 ， AFF A400 ， 或者 ASA A400HA 对，其中一个 NS224 驱动器架通过缆线连接到每个控制器上一组支持 RoCE 的端口，您必须在每个控制器上的两组端口之间重新对该架进行布线（安装额外支持 RoCE 的 PCIe 卡或 I/O 模块后）。 热添加第二个磁盘架之前。
...
=== 为 AFF A700 或 ASA A700HA 对的热添加磁盘架布线

[role="lead"]
如何为 AFF A700 或 ASA A700HA 对中的 NS224 驱动器架布线，取决于要热添加的磁盘架数量以及控制器模块上使用的支持 RoCE 的端口集数量（一个或两个）。
...
=== 为 AFF A800 或 ASA A800HA 对的热添加磁盘架布线

[role="lead"]
如何为 AFF A800 或 ASA A800HA 对中的 NS224 驱动器架布线取决于您要热添加的磁盘架数量以及控制器模块上使用的支持 RoCE 的端口集数量（一个或两个）。
...
=== 为 AFF A400 或 ASA A400HA 对的热添加磁盘架布线

[role="lead"]
如何为 AFF A400 或 ASA A400HA 对中的 NS224 驱动器架布线，取决于要热添加的磁盘架数量以及控制器模块上使用的支持 RoCE 的端口集数量（一个或两个）。
...
此操作步骤假定您的 AFF A320HA 对具有现有 NS224 磁盘架，并且您要热添加第二个磁盘架。
