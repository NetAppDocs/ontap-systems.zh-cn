...
summary: 当平台向系统控制台记录指示驱动器发生故障的警告消息时，必须热插拔驱动器。此外，操作员显示面板上的故障 LED 和故障驱动器上的故障 LED 均已亮起。 
---
= 在 AFF C800 系统中热插拔驱动器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当驱动器出现故障或需要升级时，在 AFF C800 系统中热插拔驱动器。更换过程包括识别故障驱动器、安全地将其移除，并安装新驱动器，以确保持续的数据访问和系统性能。
