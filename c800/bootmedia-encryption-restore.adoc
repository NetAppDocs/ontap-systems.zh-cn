...
summary: 在AFF C800系统中，恢复替换启动介质上的加密，以确保数据持续受到保护。替换过程包括验证密钥可用性、重新应用加密设置以及确认对数据的安全访问。 
---
= 在手动启动恢复过程后，在 AFF C800 系统中恢复启动介质上的加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在AFF C800系统中，恢复替换启动介质上的加密，以确保数据持续受到保护。替换过程包括验证密钥可用性、重新应用加密设置以及确认对数据的安全访问。
