...
summary: 当您的系统注册闪存寿命即将结束，或者所识别的 NVDIMM 总体上不健康，或者不这样做会导致系统崩溃时，您必须更换控制器模块中的 NVDIMM。 
---
= 更换 AFF C400 存储系统中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当系统记录到过多的可纠正 ECC 错误或不可纠正的 ECC 错误时，请更换 AFF C400 存储系统中的 NVDIMM，这些错误通常由单个 NVDIMM 故障引起。
...
image::../media/drw_A400_Replace-NVDIMM-DIMM_IEOPS-1009.svg[Replace the NVDIMM]
