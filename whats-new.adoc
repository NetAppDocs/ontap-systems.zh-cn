...
.AFX 2K存储系统
新的 NetApp AFX 2K 存储系统扩展了分解解决方案，旨在满足高性能 NAS 和 S3 工作负载的严格要求，包括 AI/ML 应用程序。AFX 2K 系统为需要可扩展、高吞吐量文件和对象存储的客户提供专业功能。

link:https://docs.netapp.com/us-en/ontap-afx/get-started/ontap-afx-storage.html["了解有关 AFX 系统的更多信息"] 。

.全新 Cisco 9808 交换机支持 AFX 2K
新的 Cisco 9808 交换机通过存储架连接到 AFX 2K 控制器节点。这款新交换机与 Cisco 9364D-GX2A 和 Cisco 9332D-GX2B 交换机一起，可让您构建具有两个以上节点的 ONTAP 集群。高级协议用于优化性能。该设计基于具有多个网络路径的 VLAN 标记以及技术更新配置，以确保持续运行和易于升级。
...
热插拔程序现在可用于跨多个 ONTAP 硬件平台和 ONTAP 版本的选定 I/O 模块。

*支持 ONTAP 9.18.1 GA 或更高版本：*

您可以在任何插槽中热插拔发生故障的以太网 I/O 模块，前提是其端口用于集群、HA 或客户端连接。具有任意数量节点的系统支持此过程。


NOTE: 不能热插拔具有分配给存储或 MetroCluster 连接的端口的以太网 I/O 模块。
...
*支持 ONTAP 9.17.1 或 9.18.1RC：*

为了避免双节点集群中断，只有在受损的控制器（具有故障模块的控制器）已接管健康合作伙伴的情况下，才能热插拔发生故障的集群和插槽 4 中的 HA I/O 模块。这是唯一支持的不需要停机的恢复方法。
...
AFF C800 系统已移至供应终止列表，不再适用于新订单。

link:endofavail/index.html["查看停产硬件"]。
