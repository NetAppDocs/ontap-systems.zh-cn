...
summary: 要将系统还原到完全运行状态，必须还原 NetApp 存储加密配置，并为新控制器安装许可证，然后将故障部件退回 NetApp，如套件附带的 RMA 说明中所述。 
---
= 完成 AFF C190 系统的控制器更换
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
通过恢复 NetApp 存储加密配置（如有必要）、为新控制器安装许可证并将故障部件退回 NetApp，完成 AFF C190 系统的控制器更换。



[[step-1-installing-licenses-for-the-replacement-controller-in-ontap]]
== 第1步：在ONTAP 中为_replacement_控制器安装许可证
