...
summary: 当 AFX 2K 存储系统中的 I/O 模块出现故障或需要升级以支持更高性能或附加功能时，请更换该模块。更换过程包括关闭控制器、更换出现故障的 I/O 模块、重新启动控制器以及将出现故障的部件返回 NetApp。 
---
= 更换 AFX 2K 存储系统中的 I/O 模块
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当 AFX 2K 存储系统中的 I/O 模块出现故障时，请更换该模块。更换过程包括关闭控制器、更换出现故障的 I/O 模块、重新启动控制器以及将出现故障的部件返回 NetApp。
...
NOTE: 下图显示了如何卸下垂直 I/O 模块。通常，您只需移除一个 I/O 模块。
