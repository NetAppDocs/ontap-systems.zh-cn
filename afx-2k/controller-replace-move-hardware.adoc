...
summary: 当硬件故障需要时，请更换 afx 2k 存储系统中的控制器。更换过程包括移除受损的控制器、将组件移至替换控制器、安装替换控制器并重新启动。 
---
= 更换控制器 - AFX 2K
:allow-uri-read: 
:icons: font
...
[role="lead"]
当硬件故障需要时，请更换 AFX 2K 存储系统中的控制器。更换过程包括移除受损的控制器、将组件移至替换控制器、安装替换控制器并重新启动。
...
.下一步是什么？
更换受损的 AFX 2K 控制器后，link:controller-replace-system-config-restore-and-verify.html["恢复系统设置"]。
