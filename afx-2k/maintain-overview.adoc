...
summary: 维护 AFX 2K 存储系统的硬件，以确保长期可靠性和最佳性能。执行定期维护任务，例如更换故障组件，因为这有助于防止停机和数据丢失。 
---
= 了解如何维护您的 AFX 2K 存储系统
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
维护 AFX 2K 存储系统的硬件，以确保长期可靠性和最佳性能。执行定期维护任务，例如更换故障组件，因为这有助于防止停机和数据丢失。

维护程序假设 AFX 2K 存储系统已部署为 ONTAP 环境中的存储节点。
...
NVRAM （非易失性随机存取存储器）模块可让控制器在系统断电时保护和保存飞行数据。系统 ID 存储在NVRAM模块中。更换后，控制器将使用替换NVRAM模块中的新系统 ID。
 ...
 a| 
link:nv12l-replace.html["NVRAM12-EX 模块"]
 a| 
NVRAM12-EX 是 AFX 2K 控制器的第二个 NVRAM（非易失性随机存取存储器）模块。NVRAM12-EX 模块允许控制器在系统断电时保护和保存传输中的数据。
