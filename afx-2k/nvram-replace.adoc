...
summary: 当非易失性内存出现故障或需要升级时，请更换 AFX 2K 存储系统中的 NVRAM。 
---
= 更换 AFX 2K 存储系统中的 NVRAM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当非易失性内存出现故障或需要升级时，请更换 AFX 2K 存储系统中的 NVRAM。更换过程包括关闭受损控制器、更换 NVRAM 模块或 NVRAM DIMM，以及将故障部件返回 NetApp。
...
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[卸下NVRAM12模块和DIMM]
...
. 从机柜中卸下目标NVRAM模块。
+
image::../media/drw_afx_2k_nvram_remove_replace_ieops-2881.svg[卸下NVRAM 12模块和DIMM]
...
NOTE: Override 系统 ID 仅适用于 NVRAM 模块更换，而不适用于 NVRAM DIMM 更换。
...
. 交还控制器：
+
.. 从健康控制器返回被替换控制器的存储：`_storage failover giveback -ofnode impaired_node_name_`
+
控制器将收回其存储并完成启动。
