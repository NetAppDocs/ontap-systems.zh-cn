...
summary: 如果模块出现故障并且存储系统满足所有 ONTAP 版本要求，则可以热插拔 AFX 2K 存储系统中的以太网 I/O 模块。 
---
= 在 AFX 2K 存储系统中热插拔 I/O 模块
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果模块出现故障并且存储系统满足所有 ONTAP 版本要求，则可以热插拔 AFX 2K 存储系统中的以太网 I/O 模块。
...
NOTE: 下图显示了如何卸下垂直 I/O 模块。通常，您只会移除一个 I/O 模块。
