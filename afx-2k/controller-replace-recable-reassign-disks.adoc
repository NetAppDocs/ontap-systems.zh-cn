...
summary: 将存储资源的控制权交还给替换控制器，以便您的 AFX 2K 存储系统可以恢复正常运行。 
---
= 归还 AFX 2K 存储系统中的控制器以恢复正常操作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将存储资源的控制权交还给替换控制器，以便您的 AFX 2K 存储系统可以恢复正常运行。交还过程因系统使用的加密类型而异：不加密或板载密钥管理器 (OKM) 加密。
