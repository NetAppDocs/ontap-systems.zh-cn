...
summary: '当存储系统报告由运行状况监视器警报识别的过多可更正错误更正代码 (CECC) 错误，或由单个 DIMM 故障导致的不可更正 ECC 错误（该故障会阻止存储系统启动 ONTAP）时，您必须更换控制器模块中的 DIMM。' 
---
...
[role="lead"]
当存储系统报告由运行状况监视器警报识别的过多可更正错误更正代码 (CECC) 错误，或由单个 DIMM 故障导致的不可更正 ECC 错误（该故障会阻止存储系统启动 ONTAP）时，您必须更换控制器模块中的 DIMM。
