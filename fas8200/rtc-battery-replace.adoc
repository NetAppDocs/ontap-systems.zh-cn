...
summary: 更换控制器模块中的实时时钟电池，以便依赖于准确时间同步的服务和应用程序继续运行。 
---
= 更换 FAS8200 系统中的实时时钟电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
要更换 RTC 电池，请在控制器中找到并按照特定步骤顺序进行操作。

. 如果您尚未接地，请正确接地。
. 找到 RTC 电池。
+
image::../media/drw_rxl_rtc_battery.png[卸下并更换RTC电池]
...
== 步骤 4：重新安装控制器模块并在更换 RTC 电池后重置时间和日期
