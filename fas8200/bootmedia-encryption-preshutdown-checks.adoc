...
summary: 在关闭FAS8200系统上出现故障的控制器之前，请验证加密密钥支持和状态。FAS8200系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 检查 FAS8200 系统中启动介质上的加密支持
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在关闭FAS8200系统上出现故障的控制器之前，请验证加密密钥支持和状态。此过程包括检查ONTAP版本与NetApp卷加密 (NVE) 的兼容性、验证密钥管理器配置以及备份加密信息，以确保在启动介质恢复期间的数据安全。
