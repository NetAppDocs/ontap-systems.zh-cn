...
summary: 在FAS8200系统的替换启动介质上恢复加密配置。FAS8200系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 在 FAS8200 系统中的启动介质上恢复加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在FAS8200系统的替换启动介质上恢复加密配置。此流程包括完成已启用板载密钥管理器 (OKM)、 NetApp存储加密 (NSE) 或NetApp卷加密 (NVE) 的系统更换后的步骤，以确保安全的数据访问和系统的正常运行。
