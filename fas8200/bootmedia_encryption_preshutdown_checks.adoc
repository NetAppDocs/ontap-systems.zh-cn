...
controller-- perMalink ： fas8200/ bootmedia-encryption-preshutdown-checks.html 边栏：边栏关键字： fas8200 ，板载加密密钥的预关闭检查摘要： " 在关闭受损控制器并检查板载加密密钥状态之前，您必须检查受损控制器的状态，禁用自动交还并检查系统正在运行的 ONTAP 版本。 " -- = 检查板载加密 - FAS8200 ：图标：字体： imagesdir ： ../media/
...
** 如果命令输出中未显示 <lno-dur> ，并且系统运行的是 ONTAP 9.5 ，请转至 <<Option 1: Checking NVE or NSE on systems running ONTAP 9.5 and earlier>>。
** 如果命令输出中未显示 <lno-dur> ，并且系统运行的是 ONTAP 9.6 或更高版本，请转至 <<Option 2: Checking NVE or NSE on systems running ONTAP 9.6 and later>>。
