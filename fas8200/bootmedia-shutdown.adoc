...
summary: 完成加密检查后，关闭FAS8200系统上出现故障的控制器。FAS8200系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 关闭受损控制器— FAS8200
:allow-uri-read: 
:icons: font
...
[role="lead"]
完成加密检查后，关闭FAS8200系统上出现故障的控制器。此过程包括将控制器带到 LOADER 提示符，捕获启动环境变量以供参考，以及准备控制器以更换启动介质，具体步骤因系统配置而异。
