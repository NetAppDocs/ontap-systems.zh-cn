...
summary: 要将 FAS8200 系统恢复到完全运行状态，请恢复 NetApp 存储加密配置（如有必要），安装新控制器的许可证，并按照套件附带的 RMA 说明将故障部件退回 NetApp。 
---
= 完成 FAS8200 系统的控制器更换
:allow-uri-read: 
:icons: font
...
[role="lead"]
要将 FAS8200 系统恢复到完全运行状态，请恢复 NetApp 存储加密配置（如有必要），安装新控制器的许可证，并按照套件附带的 RMA 说明将故障部件退回 NetApp。
