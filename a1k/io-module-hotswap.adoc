...
summary: 如果模块发生故障并且存储系统满足所有 ONTAP 版本要求，则可以在 AFF A1K 存储系统中热插拔以太网 I/O 模块。 
---
= 在不关闭系统的情况下热插拔 AFF A1K 系统中的 I/O 模块
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
如果模块发生故障并且存储系统满足所有 ONTAP 版本要求，则可以在 AFF A1K 存储系统中热插拔以太网 I/O 模块。

要热交换 I/O 模块，请验证存储系统是否正在运行 ONTAP 9.18.1 GA 或更高版本。然后准备好存储系统和 I/O 模块，热插拔故障模块，使更换模块联机，恢复正常运行，并将故障模块返回 NetApp。
