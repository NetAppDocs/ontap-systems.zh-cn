...
summary: 在更换 AFF A1K 系统中的控制器之前，请验证您是否符合成功更换的必要要求。这包括验证系统中的所有其他组件是否正常运行，验证您的替换控制器是否正确，以及将控制器的控制台输出保存到文本日志文件中。 
---
= 在更换 AFF A1K 系统中的控制器之前，请查看要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换 AFF A1K 系统中的控制器之前，请验证您是否符合成功更换的必要要求。这包括验证系统中的所有其他组件是否正常运行，验证您的替换控制器是否正确，以及将控制器的控制台输出保存到文本日志文件中。
...
.下一步是什么？
在查看更换 AFF A1K 控制器的要求后，需要执行此操作link:controller-replace-shutdown.html["关闭受损控制器"]。
