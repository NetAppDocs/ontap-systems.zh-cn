...
summary: 要确保AFF A1K存储系统上的数据安全、您需要验证启动介质上的加密密钥支持和状态。检查您的ONTAP版本是否支持NetApp卷加密、关闭控制器之前、请检查密钥管理器是否处于活动状态。 
---
= 检查启动介质上的加密密钥支持和状态- AFF A1K
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要确保AFF A1K存储系统上的数据安全、您需要验证启动介质上的加密密钥支持和状态。检查您的ONTAP版本是否支持NetApp卷加密(NVE)、关闭控制器之前、请检查密钥管理器是否处于活动状态。
