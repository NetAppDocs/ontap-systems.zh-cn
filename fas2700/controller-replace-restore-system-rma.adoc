...
summary: 要将 FAS2700 系统还原到完全运行状态，必须还原 NetApp 存储加密配置，为新控制器安装许可证，并将故障部件返回 NetApp。 
---
= 完成 FAS2700 系统的控制器更换
:allow-uri-read: 
:icons: font
...
[role="lead"]
要将 FAS2700 系统恢复到完全运行状态，您必须恢复 NetApp 存储加密配置（如有必要），安装新控制器的许可证，并将故障部件退回 NetApp，如套件附带的 RMA 说明中所述。
