...
summary: 检查完环境变量后，您必须完成针对已启用板载密钥管理器、 NetApp存储加密或NetApp卷加密的系统的特定步骤。AFF A320系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 在 AFF A320 系统中恢复启动介质上的加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
恢复替换启动介质上的加密。AFF A320系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。
