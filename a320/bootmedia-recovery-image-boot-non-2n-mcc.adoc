...
summary: 您必须从 USB 驱动器启动ONTAP映像，恢复文件系统，并验证环境变量。AFF A320系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。 
---
= 在 AFF A320 系统上启动恢复映像
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您必须从 USB 驱动器启动ONTAP映像，恢复文件系统，并验证环境变量。AFF A320系统仅支持手动启动介质恢复程序。不支持自动启动介质恢复。
...
. 下一个取决于您的系统配置：
+
** 如果您的系统配置了板载密钥管理器， NSE 或 NVE ，请转至 xref:bootmedia-encryption-restore.adoc[OKM ， NSE 和 NVE 的启动后介质更换步骤]
