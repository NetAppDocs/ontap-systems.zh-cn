...
summary: 当您的 ASA A800 系统注册闪存寿命即将结束或所识别的 NVDIMM 总体上不健康时，请更换 NVDIMM。否则会导致系统崩溃。 
---
= 更换 ASA A800 系统中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当您的ASA A800系统检测到闪存寿命即将结束或检测到已识别的 NVDIMM 总体上不健康时，请更换系统中的 NVDIMM；否则会导致系统崩溃。

.开始之前
* 请确保您已收到从NetApp收到的替换 NVDIMM，并且该 NVDIMM 与您的ASA A800系统兼容。
