...
summary: 将存储资源的控制权交还给替换控制器，以便您的 ASA A800 系统可以恢复正常运行。 
---
= 归还 ASA A800 系统中的控制器以恢复正常运行
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将存储资源的控制权交还给替换控制器，以便您的ASA A800系统能够恢复正常运行。返还过程会根据您系统使用的加密类型而有所不同：无加密或板载密钥管理器 (OKM) 加密。
