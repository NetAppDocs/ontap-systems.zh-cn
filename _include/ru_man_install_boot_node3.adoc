...
| 无共享磁盘 | `dreassign` <node1_sysid> -d <node3_sysid> 
|===
+
对于 ` <node1_sysid>` 值，请使用中捕获的信息 link:record_node1_information.html["记录 node1 信息"]。要获取 ` 的值 <node3_sysid>` ，请使用 `ssysconfig` 命令。
...
+
`d` reassign ` 命令仅重新分配` <node1_sysid> 为当前所有者的磁盘。
