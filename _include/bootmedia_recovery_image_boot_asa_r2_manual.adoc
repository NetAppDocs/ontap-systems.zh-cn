...
--
.. 在受损控制器上的Loader提示符处、配置受损控制器模块的IP地址：
+
`ifconfig e0M - auto`

.. 在Loader提示符处、从网络启动受损节点：
+
`netboot \http://_web_server_ip/path_to_web-accessible_directory_/<xxx_image.tgz>`

.. 从启动菜单中，选择选项 `7`。
.. 当系统询问您是否还原备份配置时、输入 `n`。
.. 在系统提示重新启动节点时、输入 `y`。
.. 按键 `ctrl-c`返回Boot Menu (引导菜单)。
+
如果启用了加密、则需要先将其还原、然后再继续还原启动映像。转到。 link:bootmedia-encryption-restore.html["恢复加密"]
