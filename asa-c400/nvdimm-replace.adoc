...
summary: 当您的系统注册闪存寿命即将结束或所识别的 NVDIMM 总体上不健康时，您必须更换控制器模块中的 NVDIMM。否则会导致系统崩溃。 
---
= 更换 ASA C400 系统中的 NVDIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当存储系统遇到过多的可纠正 ECC 错误或不可纠正 ECC 错误时，请更换 ASA C400 系统中的 NVDIMM，这些错误通常由单个 NVDIMM 故障引起。
