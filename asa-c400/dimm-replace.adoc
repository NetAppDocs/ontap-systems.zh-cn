...
summary: 当系统注册了越来越多的可更正的纠错代码或不可更正的 ECC 错误时，请更换 ASA C400 控制器模块中的 DIMM。未能更换有故障的 DIMM 可能会导致系统崩溃。 
---
= 更换 ASA C400 系统中的 DIMM 以修复内存错误
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当存储系统遇到过多的可纠正错误更正代码 (ECC) 错误或不可纠正的 ECC 错误时（这通常由单个 DIMM 故障引起），请更换 ASA C400 系统中的 DIMM。
