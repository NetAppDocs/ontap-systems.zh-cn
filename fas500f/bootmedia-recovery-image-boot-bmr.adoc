...
summary: 在 FAS500f 系统中安装新的启动介质设备后，您可以启动自动启动介质恢复过程，以从配对节点恢复配置。 
---
= 使用自动引导恢复过程在 FAS500f 系统上还原引导映像
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 FAS500f 系统中安装新的启动介质设备后，您可以启动自动启动介质恢复过程，从配对节点恢复配置。在恢复过程中，系统检查加密是否已启用，并确定正在使用的密钥加密类型。如果启用了密钥加密，系统将引导您完成恢复密钥的相应步骤。
...
.下一步行动
恢复 ONTAP 映像且节点已启动并正在提供数据后，link:bootmedia-complete-rma-bmr.html["将故障部件退回给NetApp"]。
