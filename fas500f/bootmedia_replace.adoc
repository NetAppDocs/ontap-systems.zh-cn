...
'''
PerfMalink ： fas500f/bootmedia-replace.html 边栏：边栏关键字： fas500f ，请替换启动介质摘要： " 在更换启动介质之前，您可以通过卸下控制器模块上的通风管在控制器模块中找到故障启动介质。 " -- = 更换启动介质 - FAS500f ： icons ：字体： imagesdir ： ../media/
...
要卸下用于固定启动介质的螺钉，您需要使用 1 号磁性十字螺丝刀。Due to the space constraints within the controller module, you should also have a magnet to transfer the screw on to so that you do not loose it.
