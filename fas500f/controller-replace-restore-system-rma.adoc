...
summary: 要完成 FAS500f 系统的控制器更换，请在必要时恢复 NetApp 存储加密配置，安装新控制器的许可证，并将故障部件返回 NetApp。 
---
= 完成 FAS500f 系统的控制器更换
:allow-uri-read: 
:icons: font
...
[role="lead"]
要完成 FAS500f 系统的控制器更换，请恢复 NetApp 存储加密配置（如有必要），安装新控制器的许可证，并按照套件附带的 RMA 说明将故障部件退回 NetApp。
