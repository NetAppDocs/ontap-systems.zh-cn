...
summary: 在更换 FAS500f 系统中的启动介质之前，请确保满足成功更换的必要要求。 
---
= 自动启动介质恢复的要求 - FAS500f
:allow-uri-read: 
:icons: font
...
[role="lead"]
在更换 FAS500f 中的启动介质之前，请确保满足成功更换的必要要求。这包括验证您是否有正确的替换启动介质，确认受损控制器上的 e0S (e0M 扳手) 端口没有故障，并确定是否已启用板载密钥管理器 (OKM) 或外部密钥管理器 (EKM)。
