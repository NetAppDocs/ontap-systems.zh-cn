...
summary: FAS500f 系统中的引导介质存储重要的固件和配置数据。更换过程包括卸下控制器模块，卸下受损的引导介质，在控制器模块中安装更换引导介质，然后重新安装控制器模块。 
---
= 更换启动介质以进行自动启动恢复 - FAS500f
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS500f 系统中的引导介质存储重要的固件和配置数据。更换过程包括卸下并打开控制器模块，卸下受损的引导介质，在控制器模块中安装更换引导介质，然后重新安装控制器模块。
