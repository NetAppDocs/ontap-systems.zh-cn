...
summary: 当您的系统注册了越来越多的可更正的纠错代码时，您必须更换控制器模块中的 DIMM。否则会导致系统崩溃。 
---
...
[role="lead"]
如果存储系统遇到错误，例如基于运行状况监视器警报的 CECC（可更正错误更正代码）过多错误或无法更正的 ECC 错误，则必须更换控制器中的 DIMM。这些错误通常是由单个 DIMM 故障引起的，会导致存储系统无法启动 ONTAP。
