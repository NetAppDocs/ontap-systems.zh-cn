...
summary: 将存储资源的控制权交还给替换控制器，以便 FAS500f 系统恢复正常运行。 
---
= 归还 FAS500f 系统中的控制器以恢复正常操作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将存储资源的控制权交还给替换控制器，以便 FAS500f 系统可以恢复正常运行。恢复并验证系统配置后，重新连接系统并重新分配磁盘。
...
. 在 _replacement_ 控制器上的 LOADER 提示符处，启动控制器，如果系统因系统 ID 不匹配而提示您覆盖系统 ID ，请输入 `y` ：
...
.下一步是什么？
重新连接系统并重新分配磁盘后，需要执行此操作link:controller-replace-restore-system-rma.html["完成控制器更换"]。
