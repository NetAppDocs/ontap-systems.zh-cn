...
summary: 在关闭受损控制器之前，您需要检查系统是否已启用 NetApp 卷加密 \ （ NVE\ ）或 NetApp 存储加密 \ （ NSE\ ）。If so, you need to verify the configuration. 
---
= 检查板载加密密钥— FAS500f
:allow-uri-read: 
:icons: font
...
+
** 如果命令输出中显示 <lno-dury> 或 <1osno-dury> ，则系统不支持 NVE ，请继续关闭控制器。在关闭受损控制器之前，您需要验证系统是否已启用 NetApp 卷加密（ NVE ）或 NetApp 存储加密（ NSE ）。If so, you need to verify the configuration.
