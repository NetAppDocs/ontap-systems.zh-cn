...
summary: 在关闭受损节点之前，您需要检查系统是否已启用 NetApp 卷加密 \ （ NVE\ ）或 NetApp 存储加密 \ （ NSE\ ）。如果是，则需要验证配置。 
---
...
+
** 如果命令输出中显示 <lno-dur> ，则表示系统不支持 NVE ，请继续关闭控制器。在关闭受损节点之前，您需要验证系统是否已启用 NetApp 卷加密（ NVE ）或 NetApp 存储加密（ NSE ）。如果是，则需要验证配置。
