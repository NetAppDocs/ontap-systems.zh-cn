...
summary: 定义可用于将控制器连接到磁盘架堆栈的控制器 SAS 端口对，以便在 HA 对或单控制器配置中实现四路径连接。 
---
= DS212C、DS224C 或 DS460C 四路径连接的布线工作表
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您可以使用布线工作表定义控制器 SAS 端口对，可用于将控制器连接到具有 IOM12、IOM12B 或 IOM12C 模块的磁盘架堆栈，以在 HA 对或单控制器配置中实现四路径连接。您还可以使用完成的工作表逐步完成适用于您的配置的四路径连接布线。
...
+
带有两个四端口 SAS HBA 和两个带有 IOM12、IOM12B 或 IOM12C 模块的磁盘架堆栈的配置用于工作表示例。
