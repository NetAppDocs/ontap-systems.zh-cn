...
summary: 使用此示例指导您阅读完整的工作表，并将其应用于磁盘架的控制器到堆栈的缆线连接，以实现多路径连接。 
---
= 阅读 DS212C、DS224C 或 DS460C 多路径控制器到堆栈连接的布线工作表
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
您可以使用此示例指导您如何阅读完整的工作表，并将其应用于具有 IOM12、IOM12B 或 IOM12C 模块的磁盘架的控制器到堆栈的缆线连接，以实现多路径连接。
...
+
本示例中使用的配置是多路径 HA 配置，每个控制器上有两个四端口 SAS HBA（八个 SAS 端口）和两个带有 IOM12、IOM12B 或 IOM12C 模块的磁盘架堆栈。通过跳过工作表中的每隔一个端口对来连接端口对。
