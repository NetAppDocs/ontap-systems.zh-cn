...
summary: 在双节点 MetroCluster 配置中完成 FRU 更换后，您可以执行 MetroCluster 切换操作。这将使配置恢复到正常运行状态，以前受损站点上的同步源存储虚拟机 SVM 现在处于活动状态，并从本地磁盘池提供数据。 
---
= 在 ASA A400 系统中切换回双节点 MetroCluster 配置中的聚合
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
