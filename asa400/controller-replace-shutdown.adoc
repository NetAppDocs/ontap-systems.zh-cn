...
summary: 关闭 ASA A400 存储系统中的控制器，以防止数据丢失，并确保更换控制器时的系统稳定性。 
---
= 在更换之前关闭 ASA A400 系统中受损的控制器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
关闭 ASA A400 存储系统中的控制器，以防止数据丢失，并确保更换控制器时的系统稳定性。
...
====
.下一步是什么？
关闭 ASA A400 控制器后，需要执行此操作link:controller-replace-move-hardware.html["更换控制器"]。
