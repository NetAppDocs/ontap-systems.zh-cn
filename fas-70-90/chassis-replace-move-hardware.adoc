...
summary: '当硬件故障需要更换 FAS70 或 FAS90 系统的机箱时。更换过程包括从机箱前部移除控制器，移除电源单元 (PSU)、NVRAM12、I/O 模块和空白板以及系统管理模块，然后安装更换机箱，并重新安装机箱组件。' 
---
= 更换 FAS70 或 FAS90 系统中的机箱
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当硬件故障需要更换 FAS70 或 FAS90 系统的机箱时。更换过程包括移除控制器、I/O 卡、NVRAM12 模块、系统管理模块和电源单元 (PSU)、安装更换机箱以及重新安装机箱组件。
...
.下一步是什么？
更换受损的 FAS70 和 FAS90 机箱并重新安装组件后，您需要link:chassis-replace-complete-system-restore-rma.html["完成机箱更换"]。
