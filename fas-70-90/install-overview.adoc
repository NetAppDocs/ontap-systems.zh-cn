...
查看安装存储系统和存储架所需的设备和工具。还请查看起重和安全预防措施。
...
[role="quick-margin-para"]
要准备安装系统，您需要准备好现场，检查环境和电气要求，并验证是否有足够的机架空间。然后，打开设备的包装，将其内容与装箱单进行比较，并注册硬件以获得支持权益。
...
[role="quick-margin-para"]
在开启控制器之前，打开每个 NS224 磁盘架的电源，并分配唯一的磁盘架 ID，以便每个磁盘架都有唯一的标识。然后将笔记本电脑或控制台连接到控制器，并将控制器连接到电源。
