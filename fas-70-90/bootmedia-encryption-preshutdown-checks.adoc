...
summary: 为确保 FAS70 或 FAS90 存储系统上的数据安全，您需要验证启动介质上的加密密钥支持情况及其状态。请检查您的ONTAP版本是否支持NetApp卷加密，并在关闭控制器之前检查密钥管理器是否处于活动状态。 
---
= 使用手动启动恢复过程检查 FAS70 或 FAS90 系统中启动介质上的加密支持
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要确保FAS70或FAS90存储系统上的数据安全、您需要验证启动介质上的加密密钥支持和状态。检查您的ONTAP版本是否支持NetApp卷加密(NVE)、关闭控制器之前、请检查密钥管理器是否处于活动状态。
