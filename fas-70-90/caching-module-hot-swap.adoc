...
---
= 更换 FAS70 或 FAS90 系统中的 Flash Cache 模块托架或缓存模块
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
.开始之前
* 验证您的存储系统是否具有适用于替换 Flash Cache 模块托架的操作系统。
* 确认所有其他组件都正常工作。如果没有，请联系技术支持。
...
.开始之前
* 验证替换缓存模块与故障缓存模块的容量相同，且来自同一供应商或其他受支持的供应商。
* 确认所有其他组件都正常工作。如果没有，请联系技术支持。
...
.. 将凸轮把手旋转到最远位置。
.. 将手指钩入凸轮杆开口，然后将模块从 Flash Cache 模块托架中拉出，从而将缓存模块从机箱中取出。
+
...
+
确保命令输出将状态报告为 `powered-on`且已准备好运行。
