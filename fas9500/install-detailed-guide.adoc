...
sidebar: sidebar 
keywords: FAS9500 
summary: 本页提供安装 FAS9500 系统的详细分步说明。 
---
= 按照详细说明安装 FAS9500 存储系统
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
本页提供安装 FAS9500 系统的详细分步说明。如果您需要更详细的安装说明，请使用这些说明。
...
X66033A （ 112-00438 ）， 3 米
 a| 
image::../media/oie_cable_mini_sas_hd_to_mini_sas_hd.png[迷你SAS连接器]
 a| 
存储
...
将插槽 A4 和 B4（e4b）以及插槽 A8 和 B8（e8b）中的主机端 b 端口通过线缆连接到主机交换机。
...
image::../media/icon_square_5_grey.png[标注图标4.]
 a| 
** 将缆线固定到缆线管理臂上(未显示)。
** 将电源线连接到 PSU，并将其连接到不同的电源（未显示）。PSU1 和 PSU3 为所有 A 侧组件供电，而 PSU2 和 PSU4 为所有 B 侧组件供电。
...
* 请务必检查图示箭头，以确认正确的电缆连接器拉片方向。存储模块的电缆拉片朝上，而磁盘架上的拉片朝下。
...
.步骤
. 使用以下动画或图纸将控制器连接到三个（一个堆栈包含一个驱动器架，另一个堆栈包含两个驱动器架）DS224C 驱动器架。
+
.动画电缆驱动器架
...
image::../media/oie_cable_mini_sas_hd_to_mini_sas_hd.png[Mini-SAS HD 电缆]
...
+
image::../media/drw_a900_power-on_IEOPS-941.svg[打开系统电源]
