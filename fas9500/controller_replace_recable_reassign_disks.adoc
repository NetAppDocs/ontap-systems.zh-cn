...
summary: 重新连接系统并重新分配磁盘。 
---
= 交还 FAS9500 系统中的控制器以恢复正常操作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
将存储资源的控制权交还给替换控制器，以便 FAS9500 系统可以恢复正常运行。
...
.. 输入目标系统的信息，然后选择 Collect Data。
.. 选择 Cabling 选项卡，然后检查输出。确保显示所有磁盘架，并且所有磁盘都显示在输出中，更正发现的任何布线问题。
.. 通过选择适当的选项卡检查其他布线，然后检查 Config Advisor 的输出。





NOTE: 系统 ID 和磁盘分配信息驻留在 NVRAM 模块中，该模块与控制器模块分开，不受控制器模块更换的影响。
...
如果存储系统处于 HA 对中，则在操作步骤结束时发生交还时，新控制器模块的系统 ID 将自动分配给磁盘。您必须在启动 _replacement_ 节点时确认系统 ID 更改，然后验证更改是否正确。

此操作步骤仅适用于在 HA 对中运行 ONTAP 的系统。

. 如果 _replacement_ 节点处于维护模式（显示 ` * >` 提示符），请退出维护模式并转到加载程序提示符： `halt`
. 从_replacement_节点上的 LOADER 提示符处启动节点，如果系统提示您因系统 ID 不匹配而覆盖系统 ID，请输入 `y`： `boot_ontap`
