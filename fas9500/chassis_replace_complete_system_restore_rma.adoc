...
. 执行中所述的其他运行状况检查 https://["如何执行_A_cluster-HEATY_check_with _A_script_IN ONTAP"^]
...
* 解决任何问题 https://["Active IQ 健康提醒和风险"^] (Active IQ 将需要一些时间来处理加电后AutoSupport—预期结果会有所延迟)
* 运行 https://["Active IQ Config Advisor"^]
* 使用检查系统运行状况 https://["如何执行_A_cluster-HEATY_check_with _A_script_IN ONTAP"^]
...
集群中有两个节点
 a| 
问题描述 以下命令：`node：：> cluster ha modify -configured true``node：：> storage failover modify -node node0 -enabled true`
...
集群中的节点数超过两个
 a| 
问题描述 此命令：``node：> storage failover modify -node node0 -enabled true``
