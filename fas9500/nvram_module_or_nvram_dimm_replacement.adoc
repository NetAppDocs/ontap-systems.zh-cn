...
---
= 更换 FAS9500 系统中的 NVRAM 模块或 NVRAM DIMM
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS9500 系统中的 NVRAM 模块由 NVRAM11 和 DIMM 组成。您可以更换故障的 NVRAM 模块或 NVRAM DIMM，以保持系统正常运行。

要更换出现故障的 NVRAM 模块，您必须将其从机箱中取出，将 DIMM 移至更换模块，然后将更换的 NVRAM 模块安装到机箱中。

要更换 NVRAM DIMM，必须从机箱中卸下 NVRAM 模块，更换模块中出现故障的 DIMM，然后重新安装 NVRAM 模块。
...
启动替换控制器时，必须确认系统 ID 更改，然后验证更改是否正确。
...
+
更换后，MetroCluster 配置需要几分钟时间才能恢复到正常状态，此时每个控制器都会显示已配置的状态，并启用 DR 镜像和正常模式。 `metrocluster node show -fields node-systemid` 命令输出显示旧系统 ID，直到 MetroCluster 配置恢复正常状态。
