...
sidebar: sidebar 
keywords: FAS9500 
summary: 要热插拔包含 NVRAM11 电池的放料控制器电源模块 DCPM，您必须找到故障的 DCPM 模块，将其从机箱中取出，然后安装更换的 DCPM 模块。 
---
= 更换 FAS9500 系统中的 DCPM 模块和 NVRAM11 电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
在从机箱中卸下故障模块之前，您必须手头有一个更换的 DCPM 模块。您必须在移除后五分钟内更换。将 DCPM 模块从机箱中移除后，除了故障转移到另一个控制器模块之外，拥有 DCPM 模块的控制器模块没有关闭保护。
...
+
image::../media/drw_a900_remove_NV_battery.svg[更换 NV（DCPM）电池]
