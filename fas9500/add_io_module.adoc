...
.开始之前
* 检查 https://["NetApp Hardware Universe"] 确保新 I/O 模块与您正在运行的系统和 ONTAP 版本兼容。
* 如果有多个可用插槽，请在中检查插槽优先级 https://["NetApp Hardware Universe"] 并使用适用于您的 I/O 模块的最佳配置。
...
. 对控制器 B 重复上述步骤
. 如果您安装了存储I/O模块、请按照中所述安装SAS磁盘架并为其布线 https://["热添加SAS磁盘架"^]。
...
存储模块
 a| 
安装SAS磁盘架并为其布线、如中所述https://["热添加SAS磁盘架"^]。
