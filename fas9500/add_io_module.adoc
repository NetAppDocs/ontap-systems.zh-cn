...
[role="lead"]
您可以通过在已满配置的系统中将NIC或存储适配器更换为新的，或者将新的NIC或存储适配器添加到空机箱插槽中，从而将I/O模块添加到FAS9500系统中。
...
NOTE: 确保在任何未使用的 I/O 插槽中安装空白挡板，以防止可能出现的散热问题。
...
. 如果使用插槽 3 或 7 进行网络连接，请使用 `storage port modify -node __<node name>__ -port __<port name>__ -mode network`命令转换插槽以供网络使用。
. 对控制器 B 重复上述步骤
