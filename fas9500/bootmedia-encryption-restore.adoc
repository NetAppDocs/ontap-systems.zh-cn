...
summary: 检查环境变量后，您必须完成特定于启用了 Onboard Key Manager OKM、NetApp Storage Encryption NSE 或 NetApp Volume Encryption NVE 的系统的步骤。 
---
= 在手动启动恢复过程之后，在 FAS9500 系统中恢复启动介质上的加密
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更换 FAS9500 系统中的启动介质后，通过完成适用于您的密钥管理器类型（例如 Onboard Key Manager (OKM)）的步骤来还原加密。
