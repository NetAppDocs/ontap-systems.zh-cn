...
summary: 在更换 FAS9500 系统中的启动介质之前，请验证您是否满足成功更换所需的要求。 
---
= 查看 FAS9500 系统中自动启动介质恢复的要求
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在更换 FAS9500 系统中的启动介质之前，请验证您是否满足成功更换所需的必要条件。这包括验证您是否拥有正确的替换启动介质，确认受损控制器上的 e0S（e0M 扳手）端口没有故障，并确定是否已启用板载密钥管理器 (OKM) 或外部密钥管理器 (EKM)。
