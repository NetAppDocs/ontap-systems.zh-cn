...
summary: ASA C250 系统中的引导介质存储重要的固件和配置数据。更换过程包括卸下控制器模块，卸下受损的引导介质，在控制器模块中安装更换引导介质，然后重新安装控制器模块。 
---
= 使用自动启动恢复过程更换 ASA C250 系统中的启动介质
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
ASA C250 系统中的引导介质存储重要的固件和配置数据。更换过程包括卸下并打开控制器模块，卸下受损的引导介质，在控制器模块中安装更换引导介质，然后重新安装控制器模块。
