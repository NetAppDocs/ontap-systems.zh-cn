...
summary: 当平台向系统控制台记录指示驱动器发生故障的警告消息时，请热插拔 ASA C250 系统中的驱动器。此外，操作员显示面板上的故障 LED 和故障驱动器上的故障 LED 都会亮起。 
---
= 在 ASA C250 系统中热插拔驱动器
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当 ASA C250 系统中的驱动器出现故障时，可对其进行热插拔。更换过程包括识别故障驱动器、将其安全移除以及安装新驱动器，以确保持续的数据访问和系统性能。
