...
summary: 当平台向系统控制台记录指示驱动器发生故障的警告消息时，请更换 ASA C250 系统中的驱动器。此外，操作员显示面板上的故障 LED 和故障驱动器上的故障 LED 都将点亮。 
---
= 更换 ASA C250 系统中的驱动器
:allow-uri-read: 
:icons: font
...
[role="lead"]
当 ASA C250 系统中的驱动器出现故障时，请更换该驱动器。更换过程包括识别故障驱动器、安全地将其移除，以及安装新驱动器，以确保持续的数据访问和系统性能。
