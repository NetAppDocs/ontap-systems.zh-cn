...
summary: 您必须验证机箱的 HA 状态，切换回聚合，并将故障部件退回 NetApp，以按照套件随附的 RMA 说明完成 FAS2820 系统的机箱更换。 
---
= 完成 FAS2820 系统的机箱更换
:allow-uri-read: 
:icons: font
...
[role="lead"]
验证机箱的 HA 状态，切换回聚合，并将故障部件返回 NetApp，以按照套件随附的 RMA 说明完成 FAS2820 系统的机箱更换。
...
. 通过将摇杆开关切换为 *ON* 来打开 PSU，并等待控制器完全通电。
. 开机后、检查机箱和控制器的正面和背面是否有任何故障指示灯。
. 通过 SSH 连接到节点的 SP 或 BMC IP 地址。这与用于关闭节点的地址相同。
