...
summary: 更换 FAS2820 系统控制器模块中的实时时钟电池，以确保依赖精确时间同步的服务和应用程序继续正常运行。 
---
= 更换 FAS2820 系统中的实时时钟电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
更换 FAS2820 系统控制器模块中的实时时钟 (RTC) 电池，以确保依赖精确时间同步的服务和应用程序继续正常运行。
...
通过在控制器内部找到 RTC 电池并按照特定步骤顺序进行操作，更换 RTC 电池。
