...
summary: 当 FAS2820 系统的控制器遇到错误时，您必须更换控制器中的 DIMM，例如基于 Health Monitor 警报的过多 CECC（可纠正错误更正代码）错误或不可纠正的 ECC 错误，这些错误通常由单个 DIMM 故障引起，导致存储系统无法启动 ONTAP。 
---
= 更换 FAS2820 系统中的 DIMM 以修复内存错误
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
当 FAS2820 系统的控制器遇到错误时，您必须更换控制器中的 DIMM，例如基于 Health Monitor 警报的过多 CECC（可纠正错误更正代码）错误或不可纠正的 ECC 错误，这些错误通常由单个 DIMM 故障引起，导致存储系统无法启动 ONTAP。
