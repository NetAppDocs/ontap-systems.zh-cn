...
summary: 通过恢复 NetApp Storage Encryption 或 Volume Encryption 配置（如有必要）、为更换的控制器安装许可证以及按照随套件附带的 RMA 说明中的描述将故障部件退回给 NetApp，使您的 FAS2820 系统恢复完全运行。 
---
= 完成 FAS2820 系统的控制器更换
:allow-uri-read: 
:icons: font
...
[role="lead"]
通过恢复 NetApp Storage Encryption 或 Volume Encryption 配置（如有必要）、为更换的控制器安装许可证以及按照随套件附带的 RMA 说明中的描述将故障部件退回给 NetApp，使您的 FAS2820 系统恢复完全运行。
