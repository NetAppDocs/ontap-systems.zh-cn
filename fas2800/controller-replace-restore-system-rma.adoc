...
summary: 如套件附带的 RMA 说明中所述，通过恢复 NetApp 存储加密或卷加密配置，并安装替换控制器的许可证，以及将故障部件退回给 NetApp，将系统恢复到完整运行状态。 
---
= 完整的系统恢复—FAS2820
:allow-uri-read: 
:icons: font
...
[role="lead"]
按照套件附带的RMA说明中的说明、还原NetApp存储加密或卷加密配置(如果需要)、安装更换用的控制器的许可证并将故障部件退回给NetApp、从而将系统还原到完全运行状态。
