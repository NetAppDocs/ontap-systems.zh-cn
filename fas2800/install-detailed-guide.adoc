...
summary: 此过程提供了有关安装 FAS2820 存储系统的详细分步说明。 
---
= 按照详细说明安装 FAS2820 系统
:allow-uri-read: 
:icons: font
...
[role="lead"]
此过程提供有关准备、安装、布线和接通 FAS2820 存储系统电源的详细分步说明。如果您希望获得详尽的说明而非简短的快速指南，请使用这些步骤。
...
NOTE: 如果您的系统没有外部磁盘架，请参见 link:../sas3/install-cabling-worksheets-examples-fas2600.html#fas2820-platform-in-a-multipath-ha-configuration-with-no-external-shelves["无外部磁盘架的多路径 HA 配置中 FAS2820 的布线说明"]。
...
使用选项 1（如果启用了网络发现）或选项 2（如果未启用网络发现）完成存储系统设置和配置。
...
.选项 1 ：如果启用了网络发现
--
如果您的笔记本电脑上已启用网络发现，请使用自动群集发现完成存储系统设置和配置。

.步骤
. 打开磁盘架电源，并使用此步骤开始时的动画设置磁盘架 ID。
...
. 创建帐户或登录到您的帐户。
+
.. 选择 https://mysupport.netapp.com["mysupport.netapp.com"]
.. 如果您需要创建账号或登录账号，请选择_创建账号_。
