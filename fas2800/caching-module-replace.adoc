...
summary: 当 FAS2820 系统的控制器模块注册单个 AutoSupport 消息（表明该模块已离线）时，必须更换其中的缓存模块。如果不更换出现故障的模块，则存储系统会出现性能下降。 
---
= 更换 FAS2820 系统中的缓存模块
:allow-uri-read: 
:icons: font
...
[role="lead"]
当 FAS2820 系统的控制器模块注册了一条模块已离线的 AutoSupport (ASUP) 消息时，必须更换缓存模块。如果不更换出现故障的模块，则存储系统会出现性能下降。
