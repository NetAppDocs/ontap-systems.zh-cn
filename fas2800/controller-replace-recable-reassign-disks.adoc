...
summary: 要完成控制器更换并将 FAS2820 系统恢复到完全运行状态，您必须重新连接存储、确认磁盘重新分配、还原 NetApp Storage Encryption 配置（如有必要），并为新控制器安装许可证。 
---
= 归还 FAS2820 系统中的控制器以恢复正常操作
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要完成更换过程并将 FAS2820 系统恢复为完全运行状态，您必须重新连接存储、确认磁盘重新分配、还原 NetApp Storage Encryption 配置（如有必要），并为新控制器安装许可证。
