...
summary: 要更换 FAS2820 系统中的 NVMEM 电池，您必须卸下控制器模块，将其打开，更换电池，然后关闭并重新安装控制器模块。 
---
= 更换 FAS2820 系统中的 NVMEM 电池
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
要更换 FAS2820 系统中的 NVMEM 电池，您必须卸下控制器模块，将其打开，更换电池，然后关闭并重新安装控制器模块。

如果系统中的任何其他组件无法正常工作，请联系 https://mysupport.netapp.com/site/global/dashboard["NetApp 支持"]。
...
NOTE: 如果系统处于"等待回馈"状态，或者系统未正确接管或停止（未提交的数据），则从控制器断电时，面板上的绿色 NV LED 开始闪烁。如果受损的控制器模块未被配对控制器模块成功接管，请联系 https://mysupport.netapp.com/site/global/dashboard["NetApp 支持"]

+
** 如果从机箱中卸下控制器模块后、绿色NV状态LED开始闪烁：
+
*** 确认控制器已被配对控制器模块完全接管，或受损控制器显示 _waiting for giveback_。如果是这样，您可以忽略闪烁的 LED，继续从机箱中取出受损的控制器。


** 如果绿色 NV LED 熄灭，您可以完成从机箱中移除受损控制器的操作。
