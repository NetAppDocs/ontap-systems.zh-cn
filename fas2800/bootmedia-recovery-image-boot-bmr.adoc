...
summary: 在 fas2800 系统中的替换启动介质上恢复ONTAP映像。该过程涉及从 LOADER 提示符启动启动恢复过程并监视恢复过程直到其完成并且节点重新启动。 
---
= 从合作伙伴节点自动恢复启动介质 - FAS2800
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
在 FAS2800 系统中安装新的启动介质设备后，您可以启动自动启动介质恢复过程，从合作伙伴节点恢复配置。在恢复过程中，系统会检查是否已启用加密，并确定正在使用的密钥加密类型。如果已启用密钥加密，系统将引导您完成相应的恢复步骤。
