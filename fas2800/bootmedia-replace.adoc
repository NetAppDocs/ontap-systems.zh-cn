...
summary: FAS2820 系统中的启动介质存储基本固件和配置数据。要更换此启动介质，需要卸下并打开受损的控制器模块，找到并更换启动介质，将启动映像传输到 USB 驱动器，然后启动控制器。 
---
= 使用手动启动恢复过程更换 FAS2820 系统中的启动介质
:allow-uri-read: 
:icons: font
:imagesdir: ../media/
...
[role="lead"]
FAS2820 系统中的启动介质存储基本固件和配置数据。要更换此驱动器，请卸下并打开受损的控制器模块，找到并更换启动介质，将启动映像传输到 USB 驱动器，将 USB 驱动器插入控制器，然后启动控制器。
...
+
.. 使用 https://kb.netapp.com/on-prem/ontap/DM/Encryption/Encryption-KBs/How_to_determine_if_the_running_ONTAP_version_supports_Data_At_Rest_Encryption["如何确定您的 ONTAP 版本是否支持 NetApp 卷加密 (NVE)"^]确定当前是否支持卷加密。
+
